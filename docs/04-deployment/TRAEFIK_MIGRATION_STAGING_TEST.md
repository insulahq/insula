...
git clone https://github.com/insulahq/hosting-platform.git /opt/platform
cd /opt/platform
...
These were caught in the 2026-05-14 `testing.example.test` fresh
...
4. **Smoke harness false fails on single-node** —
   ...
   `*.staging.example.test` + multi-node expectations. Skip these
