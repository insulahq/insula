...
- K8s manifest generation from workload/domain configuration (dynamic per-tenant pod provisioning)
...
│   └── tenant-panel/         # React 18 + Vite + shadcn/ui
...
| 17 | POST/GET/PUT/DELETE /api/v1/admin/tenants | `backend` `phase:1` | Backend |
| 18 | POST/GET/PUT/DELETE /api/v1/admin/tenants/{id}/domains | `backend` `phase:1` | Backend |
| 19 | POST /api/v1/admin/tenants/{id}/namespaces (auto-provision) | `backend` `phase:1` | Backend |
| 20 | GET /api/v1/admin/tenants/{id}/status (namespace health) | `backend` `phase:1` | Backend |
...
## Week 7-8: Tenant Panel MVP (Frontend + Backend)
...
| 46 | FileBrowser deployment: per-tenant pod with Longhorn PV mount | `infrastructure` `phase:1` | DevOps |
...
```
POST /api/migration/start          — Begin migration job (source: Plesk host/creds, target: tenant_id)
...
2. **Commit with meaningful messages:**
   ...
   ```bash
   git commit -m "feat(backend): add full-text search to GET /api/v1/admin/tenants

   Adds MySQL FULLTEXT index on (name, primary_email, secondary_email).
