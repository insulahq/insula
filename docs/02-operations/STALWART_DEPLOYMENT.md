...
./scripts/generate-stalwart-secret.sh \
  --hostname=mail.staging.example.test \
...
# to admin.<env>.example.test/login unless you have that cookie.
curl -ILks https://mail-admin.staging.example.test/ | head -5

# Expected: HTTP/2 302 ; location: https://admin.staging.example.test/login?...
```
...
Reproduced on 2026-04-23 staging rebootstrap. v0.15.5 accepts the exact same config and boots cleanly (verified on the drill VM at 192.0.2.58).
...
- **MX records**: DNS (PowerDNS — separate project per ADR-022) must
  have `mail.staging.example.test` pointing at the staging IP and
