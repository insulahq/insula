...
```bash
ssh root@staging1.example.test
...
./scripts/migrate-mail-pg.sh \
  --remote root@staging1.example.test \
  --source mail-pg \
  --target mail-pg-17 \
...
5. **Saves a snapshot of the source app-credentials Secret to
   ...
   ```bash
   scp root@staging1.example.test:/tmp/mail-pg-app-credentials.before-mail-pg-17.yaml ./
   ```
...
echo "QUIT" | openssl s_client -connect staging1.example.test:587 \
  -starttls smtp -quiet 2>/dev/null | head -3

# 5c. IMAPS handshake?
echo "a logout" | openssl s_client -connect staging1.example.test:993 \
...
curl -k --max-time 10 -u "test@<domain>:<password>" \
  https://stalwart.staging.example.test/.well-known/jmap
