...
`platform.example.test/mail-standby=true`. Platform-api's
...
3. Wait one `dr-watcher` tick (~30s). The startup reconciler runs and:
   ...
   - Adds `platform.example.test/mail-standby=true` label to
...
```bash
for node in $(kubectl get nodes -l platform.example.test/mail-standby=true -o jsonpath='{.items[*].metadata.name}'); do
  echo "=== $node ==="
