...
> **Authoritative bundle format:** [`BACKUP_COMPONENT_MODEL.md`](../architecture/BACKUP_COMPONENT_MODEL.md).
> **Architectural decisions:** [`ADR-028`](../architecture/adr/ADR-028-backup-architecture.md).
...
Consolidated here from earlier drafts — see [ADR-028](../architecture/adr/ADR-028-backup-architecture.md) for rationale.
...
- Full-platform disaster recovery — owned by operator tooling (Velero).
...
**Per-message, per-folder, and date-range restore are explicitly not supported** — see §1.2 and [ADR-028](../architecture/adr/ADR-028-backup-architecture.md). Operators needing targeted recovery use the pre-restore snapshot retained from step 3.
