...
| **Ingress Controller** | Traefik v3 | Latest | Native IngressRoute + middlewares; replaced NGINX Ingress (ADR-038) |
| **Traffic Routing** | DNS-based ingress routing (Traefik DaemonSet + external PowerDNS multi-A) | — | No hoster lock-in, no Floating IP, automatic DNS failover. See ADR-014. |
...
| **WAF** | ModSecurity CRS sidecar on Traefik + CrowdSec | Latest | OWASP CRS v4 per-route; CrowdSec L4/L7 bans (ADR-038) |
...
| Component | Choice | Version | Rationale |
| --- | --- | --- | --- |
| **Container Registry** | GHCR | — | Harbor was never deployed; GHCR hosts all platform images |
...
| Component | Choice | Version | Rationale |
| --- | --- | --- | --- |
| **Platform DB** | PostgreSQL 18 via CNPG operator | Latest | Platform DB is PostgreSQL (CNPG Cluster CR); tenant DBs are per-tenant MariaDB/PostgreSQL StatefulSets |
| **PostgreSQL** | CloudNativePG | Latest | Cloud-native, excellent HA features, operator-managed |
| **Caching** | In-memory LRU (per replica) | — | Redis removed M14 (2026-05); no external cache dependency |
...
| Component | Choice | Version | Rationale |
| --- | --- | --- | --- |
| **Metrics & health** | Built-in modules (`metrics`, `node-health`, `cluster-health`) + admin Monitoring page | — | Prometheus/Loki/Grafana/Alertmanager **not deployed** — open decision, see `docs/roadmap/ROADMAP.md` R2 |
