...
> **Reality note (2026-06-07):** Implemented with these deviations: **fail2ban
> runs host-level** (installed by `bootstrap.sh`, surfaced in the SSH-Lockdown
> admin UI) — not as a DaemonSet; **sealed-secrets controller** is installed by
> bootstrap (Helm), not via `k8s/base/`; service-to-service **mTLS is not
> implemented** (NetworkPolicy segmentation only — `docs/roadmap/ROADMAP.md` R12);
> automated secret-rotation CronJobs are designs, not deployed. WAF/CrowdSec
> specifics live in ADR-038 and the Web Defense docs.
