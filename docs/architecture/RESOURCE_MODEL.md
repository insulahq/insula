...
**Authoritative reference:** [ADR-037](adr/ADR-037-burstable-cpu-resource-model.md)
...
When no `resourceShare` is declared on a multi-component app, an even split with default floors (50m / 64Mi) is used. This is defence-in-depth: the bug that motivated [ADR-037](adr/ADR-037-burstable-cpu-resource-model.md) cannot recur even on un-updated manifests.
