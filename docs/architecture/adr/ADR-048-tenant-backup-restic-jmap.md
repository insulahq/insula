# ADR-048 — Tenant Backup v2: restic + JMAP + Maildir

**Status:** Proposed (2026-05-09) · Phase 1 (restic) shipped 2026-05-10; mail path superseded by ADR-044 (IMAP MULTIAPPEND)
**Note:** Renumbered from duplicate "ADR-036" on 2026-06-07 (ADR-036 = custom deployments; ADR-047 = DKIM A/B selectors).
...
> **Evolution note (2026-07-07).** Primitive 3 has since gained four properties, keeping the "no new component; dumps live in the files snapshot" framing intact:
> - **MongoDB coverage.** `mongodump --archive --gzip` → `predump-<db>-<bundleId>.archive.gz` (restored via `mongorestore --archive --gzip --drop`, which recreates namespaces). Mongo was silently unsupported before.
> - **Hot-consistent SQL dumps.** `mysqldump`/`mariadb-dump` now run with `--single-transaction --quick --routines --triggers` — a consistent hot snapshot with **no table lock and no write-downtime** on the live tenant DB (`pg_dump` was already MVCC-consistent).
> - **Free-space guard.** The logical dump is skipped (nothing written) when the DB pod's data volume is **>= 90% full** or **< 200 MiB free**, so a dump can never `ENOSPC` the live database. The raw-files floor still covers that DB.
> - **Visible, loud-but-non-blocking summary.** Per-database dump outcome is recorded in `backup_jobs.database_dumps` (JSONB; contract type `BackupDatabaseDumps` in `packages/api-contracts/src/tenant-bundles.ts`), surfaced on `GET /admin/tenant-bundles/:id`. Per-db status `dumped` / `degraded` / `failed`; bundle roll-up `ok` / `degraded` / `none` + `remediation`. This is a **separate dimension** from the bundle's `status`: a `completed` bundle can carry a `degraded`/`failed` `database_dumps` and stays fully restorable via the raw-files floor — a degraded/failed logical dump never flips the bundle to `partial` and never blocks restore. Operator runbook: `docs/operations/DATABASE_RECOVERY.md`.
