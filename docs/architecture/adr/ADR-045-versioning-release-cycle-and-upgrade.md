...
| 12 | Staging cluster mode | **Mode A only, Phase 1.** Mode A = Flux watches `development` branch tip (every-commit bleeding-edge smoke). Mode B (Flux pins latest `-rc.N` tag) deferred. **Update 2026-06-21 (R22):** the re-pin half of Mode B is implemented — the Flux re-pin accepts an `-rc.N` tag when `auto_update_include_prereleases` is on, so staging can pin a release-candidate tag; *apply* stays operator-gated (no staging auto-apply loop). |
...
| 15 | DR ownership | **No standalone DR script.** DR is absorbed into `platform-ops` as `platform-ops dr restore`. `dr-drill.yml` extended to exercise this path. _(Superseded 2026-06-09: the rehearsal harness `scripts/dr-drill.sh` now runs **locally or on a private host**, not in public CI — the age key + report token are too sensitive for public-repo Actions secrets. The `platform-ops dr restore` ownership is unchanged. See [DR_DRILL.md](../../operations/DR_DRILL.md).)_ |
