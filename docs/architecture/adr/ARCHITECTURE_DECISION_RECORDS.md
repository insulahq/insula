...
**Status:** Living index — newer ADRs live as individual `ADR-0xx-*.md` files in this directory
**Last Updated:** 2026-06-07 (docs restructure; renumbered duplicate ADR-036b → ADR-047)
**Owner:** Architecture & Engineering Team

> **Implementation-status note (2026-06-07):** Early in-index ADRs (001–025) record
> planning-era decisions; several were overtaken by later ADRs or implementation:
> ADR-002 (Harbor) — **not deployed**, GHCR is the registry; ADR-003 (MariaDB) —
> platform DB is **PostgreSQL/CNPG**; ADR-006 (Redis caching) — **Redis removed**
> (in-memory LRU); ADR-008 (Prometheus/Grafana/Loki) — **not deployed**, see
> `docs/roadmap/ROADMAP.md` R2; ADR-010 (NGINX Ingress) — superseded by **Traefik**
> (ADR-038); ADR-007 (multi-region) — **descoped 2026-06-07**. Individual ADR files
> carry their own status lines and are authoritative.
...
Symptom: `curl http://100.64.0.198:8082/` from ns2 returns `000` (connection refused/timeout)
...
- NOTIFY from ns1's public IP is rejected by ns2 (`allow-notify-from` only accepts `100.64.0.198`)
...
2. **ns2 PowerDNS must use `network_mode: host`.** Even with `userland-proxy: false`, NetBird's
   ...
   the original WireGuard source IP (`100.64.0.198`) end-to-end.
...
5. **Old public-IP autoprimary must be removed** if it was ever registered on ns2. Having both
   `192.0.2.142` and `100.64.0.198` as autoprimaries allows the old public-IP NOTIFY path
   to bypass `allow-notify-from`. Remove with:
   `pdnsutil remove-autoprimary 192.0.2.142 ns1.example.test`
...
- DNS: `netbird.example.test. IN A 192.0.2.142` (ns1)
- DNS: `netbird.example.test. IN A 192.0.2.29` (ns2)

**Clients connect to `https://netbird.example.test`:**
...
- DNS: `netbird.example.test. IN A 100.76.100.100` (floating IP)
...
3. Update DNS: `netbird.example.test. IN A <floating-ip>`
...
## ADR-047: DKIM — fixed alternating A/B selectors (dkim-1/dkim-2)

See [ADR-047-dkim-ab-selectors.md](ADR-047-dkim-ab-selectors.md).

---

## ADR-048: Tenant Backup v2 — restic + JMAP + Maildir

See [ADR-048-tenant-backup-restic-jmap.md](ADR-048-tenant-backup-restic-jmap.md).
