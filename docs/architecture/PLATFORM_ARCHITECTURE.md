...
> **Reality note (2026-06-07):** Written pre-implementation; the architecture
> holds but specific components changed: platform DB is **PostgreSQL via CNPG**
> (not MariaDB — tenant add-on DBs remain MariaDB/PostgreSQL per tenant), platform
> caching is **in-memory LRU** (Redis removed 2026-05), ingress is **Traefik v3**
> (ADR-038, not NGINX), registry is **GHCR** (Harbor never deployed), mail is
> **Stalwart** (ADR-030, not Docker-Mailserver). Cross-check details against
> `docs/architecture/TECH_STACK_SUMMARY.md` and the ADR index.
...
- [FRONTEND_DEPLOYMENT_ARCHITECTURE.md](FRONTEND_DEPLOYMENT_ARCHITECTURE.md) — Admin/tenant panel deployment
- [FRONTEND_INGRESS_CONFIGURATIONS.md](FRONTEND_INGRESS_CONFIGURATIONS.md) — Kubernetes Ingress configurations
