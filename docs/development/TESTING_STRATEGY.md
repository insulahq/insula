...
| Component | Aspirational target | Enforced threshold (vitest) |
| --- | --- | --- |
| **Backend API** | 80% | ~29% statements / 37% branches / 49% functions (see `backend/vitest.config.ts`) |
| **Frontend UI** | 75% | not enforced |

> **Why the enforced numbers are low (2026-06-07):** much of the backend is
> Kubernetes orchestration that is deliberately **integration-tested against
> live clusters** (100+ `scripts/integration-*.sh` harnesses + smoke tests)
> rather than unit-mocked. Unit-coverage metrics undercount real coverage.
> Raising enforced thresholds is welcome, but never at the cost of replacing
> live-cluster integration tests with mocks.
