...
- ns1 sends NOTIFY to ns2 (192.0.2.29) on every zone add/change/delete
- ns2 pulls full zone via AXFR
- Propagation: < 5 seconds (not minutes)

**Nameservers given to customers:**
- `ns1.example.test` (192.0.2.142) — primary, authoritative
- `ns2.example.test` (192.0.2.29) — secondary, read-only
...
# Verify DS chain
dig @192.0.2.142 acme.com DS +dnssec
```
...
PUT /api/v1/tenants/{tenantId}/domains/{domainId}/records

# Check DNS propagation (checks both ns1 and ns2)
GET /api/v1/tenants/{tenantId}/domains/{domainId}/dns-status
```
...
├── ns1.example.test (192.0.2.142) — Primary
└── ns2.example.test (192.0.2.29) — Secondary
...
- [ ] **Verify DNS** — `dig @192.0.2.142 example.test SOA` returns SOA
