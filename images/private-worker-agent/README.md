...
docker run -d \
  ...
  ghcr.io/insulahq/private-worker-agent:latest
```
...
services:
  ...
  private-worker-agent:
    image: ghcr.io/insulahq/private-worker-agent:latest
    restart: unless-stopped
    depends_on:
      - my-app
    environment:
...
```bash
docker pull ghcr.io/insulahq/private-worker-agent:latest
