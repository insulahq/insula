...
- **The two tenant-user password endpoints no longer accept a password.**
  ...
  `POST /api/v1/tenants/{id}/users/{userId}/reset-password` rejects
  `new_password` — both with `400 INVALID_FIELD_VALUE`. They are rejected
  rather than ignored so a script cannot appear to succeed while handing a
  user a password that was never set. The generated password comes back in
  the response instead: `data.generatedPassword` on create, and `data.password`
  on reset (which now answers `200` with a body, where it previously answered
  `204` with none). Scripted callers must read the password out of the
  response; the admin and tenant panels already do.

### Changed
- **Passwords for additional tenant users are now generated by the platform,
  the same way the main tenant login always has been.** The "Add User" form no
  longer has a password box — a strong password is generated when the user is
  created and shown once, with a copy button. This closes the gap where the
  main tenant account got a strong random password but every teammate added
  afterwards got whatever the person filling in the form chose.
- **"Reset password" now regenerates the password instead of letting you type
  a new one.** The dialog asks for confirmation and then shows the new
  password once. Neither an administrator nor a tenant admin can pin a chosen
  password onto someone else's account, and a reset can no longer quietly
  re-use a password the user already had. As before, the user is not emailed —
  pass the new password on over a secure channel — and their existing sessions
  are ended immediately.

### Fixed
- **A new tenant's login is now named after the contact person, not the
  company.** Creating a tenant for "Acme Corp" with contact "Jane Doe" made a
  login called "Acme Corp", so the team list, the audit log's actor column and
  outgoing mail all showed the company where a person's name belonged. New
...
- **The Banned IPs filters "Static bans only" and "Auto-bans only" did
  ...
  be forgotten the same way. This is why an operator's static ban appeared to
  be missing: it was in the LAPI, behind 16,220 unfiltered community rows.
...
- **The Banned IPs tab lists only PLATFORM decisions.** Production held 16,220
  ...
  combined table buried every operator action. Platform means operator bans,
  static bans, the auto-ban scheduler **and** the log-processing agent's own
  scenario detections (`origin: crowdsec`). The community feed has its own
  paginated, searchable viewer, reachable from the LAPI tile and from a banner
  on the Banned IPs tab that states how many IPs it is blocking. Each row in the
  viewer has an **Exclude** button that adds the IP to the allowlist, which
  overrides every ban regardless of origin — a far smaller hammer than turning
  the whole feed off.
- **The community blocklist is now opt-in and OFF by default.** It bans tens of
  ...
  on 2026-09-06 while ordinary visitors and PageSpeed were unaffected).

    Switching it off does three things together, because any one alone leaves it
    half-applied: it writes `DISABLE_ONLINE_API` to a ConfigMap Flux will not
    revert, **purges the decisions already loaded** (disabling only stops the
    refresh, and CAPI TTLs run to 144h), and **rolls the LAPI** so the running
    process picks the setting up — the image reads it only at startup. The
    platform creates the ConfigMap itself, since the annotation that protects it
    from Flux also stops Flux ever creating it. Operator bans, the allowlist and
    the static blocklist are untouched either way.
