...
export function extractResourceInfo(path: string): {
  ...
  // /api/v1/tenants/:tenantId/:subResource/:subId
  ...
  // the tenantId. Map cleanly to resourceType='restore_cart' so the
  ...
  if (segments[0] === 'admin' && segments[1] === 'restores' && segments[2] === 'carts') {
    return {
      ...
      // tenantId resolved by the cart row, not derivable from the URL
    };
  }
  ...
  // /tenants/:tenantId/:subResource/:subId
