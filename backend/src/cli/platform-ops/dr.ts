...
const DR_HELP: Readonly<Record<string, string>> = {
  ...
  restore: [
    ...
    '',
    '  RUN THIS ON THE NODE HOSTING THE CNPG PRIMARY.',
    '  Postgres accepts connections from pods and from its own node only, so a',
    '  host-run restore cannot reach the database from any other node — it just',
    '  times out. Find the node with:',
    '    kubectl -n platform get pods -l cnpg.io/cluster=system-db -o wide',
    '  and use the one whose role is primary. kubectl itself works from any node;',
    '  only the direct database connection is restricted.',
    '',
    '  Exit: 0 ok | 1 runtime failure | 2 usage error',
  ].join('\n'),
  rescue: [
