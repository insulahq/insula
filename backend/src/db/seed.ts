...
// Hosting Plans
//
// Defaults shipped to fresh installs only — the ON CONFLICT clause below
// keeps existing rows untouched (operators can edit live plans via the
// admin UI without re-seeding clobbering them).
//
// Resources:                CPU   RAM     Disk   Users  Mailboxes  AI/wk
//   Starter                 0.25  256 MB  2 GB   1       5         —
//   Business                1.00  1 GB    5 GB   3      10         $3
//   Premium                 2.00  2 GB    10 GB  10     10         $10
//
...
// are GB (0.25 == 256 MiB).
// NOTE: `code` is the stable identifier; the display `name` was retiered in
// migration 0064 so the ladder ascends (Starter < Premium < Ultimate). Hence
// code 'business' is named "Premium" and code 'premium' is named "Ultimate".
// maxMailboxSizeMb is the per-mailbox size ceiling (MB).
...
if (process.env.NODE_ENV !== 'production') {
  ...
  await db.insert(oidcProviders).values([
    ...
    },
    {
      ...
      issuerUrl: dexIssuer,
      clientId: 'hosting-platform-tenant',
