...
export async function deploymentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/tenants/:tenantId/deployments/:id/db-users', async (request, reply) => {
    const { tenantId, id } = request.params as { tenantId: string; id: string };
    const body = (request.body ?? {}) as { username?: string; database?: string; password?: unknown };
    if (!body.username) {
      throw new ApiError('MISSING_REQUIRED_FIELD', 'Username is required', 400, { field: 'username' });
    }
    // REJECT a submitted password rather than ignoring it.
    //
    // Both tenant-panel screens used to generate a password client-side, send
    // it here, and then display the value they had generated. This endpoint
    // silently discarded it and applied its own — so every database user
    // created through the panel had a password nobody had ever been shown, and
    // the failure surfaced only as "Access denied" from the tenant's
    // application. A 400 makes that divergence impossible to reintroduce
    // quietly: a client that sends a password now finds out.
    if (body.password !== undefined) {
      throw new ApiError(
        'INVALID_FIELD_VALUE',
        'Passwords are generated by the server; do not submit one. Use the password returned in the response.',
        400,
        { field: 'password' },
      );
    }
  ...
  app.post('/tenants/:tenantId/deployments/:id/db-users/:username/password', async (request) => {
    ...
    // See the create route: a submitted password was silently ignored while the
    // panel displayed it, so a "regenerated" password never matched the account.
    if ((request.body as { password?: unknown } | undefined)?.password !== undefined) {
      throw new ApiError(
        'INVALID_FIELD_VALUE',
        'Passwords are generated by the server; do not submit one. Use the password returned in the response.',
        400,
