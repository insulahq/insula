...
import { clientsStatusStampHook } from './db-tenants-stamp.js';
import {
  domains, cronJobs, mailboxes, emailAliases, deployments, tenants,
} from '../../../db/schema.js';
import type { HookCtx, Transition } from '../registry/index.js';

interface DbCall {
  table: 'domains' | 'cronJobs' | 'mailboxes' | 'emailAliases' | 'deployments' | 'tenants';
  op: 'update' | 'delete';
...
function ctx(transition: Transition, tenantId = 'c1'): HookCtx {
...
describe('Phase 3 registry topology', () => {
  ...
  // tenants-status-stamp hook's `after` list named hooks that don't
...
describe('tenants-status-stamp hook', () => {
  let fake: ReturnType<typeof makeFakeDb>;
  beforeEach(() => { fake = makeFakeDb(); });
  ...
  it('sets status=suspended + suspendedAt on suspended', async () => {
    ...
    expect(last.table).toBe('tenants');
