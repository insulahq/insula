...
describe('pv-cleanup-released hook', () => {
      ...
      pvSequence: [[
        {
          ...
          spec: { claimRef: { namespace: 'tenant-test' } },
  ...
  it('discovers a late-binding PV across multiple poll cycles', async () => {
    ...
    const ctx = makeCtx({
      pvSequence: [
        [],
        [{ metadata: { name: 'pvc-late' }, spec: { claimRef: { namespace: 'tenant-test' } }, status: { phase: 'Bound' } }],
        [{ metadata: { name: 'pvc-late' }, spec: { claimRef: { namespace: 'tenant-test' } }, status: { phase: 'Bound' } }],
        [{ metadata: { name: 'pvc-late' }, spec: { claimRef: { namespace: 'tenant-test' } }, status: { phase: 'Released' } }],
      ],
      deletedPvs,
      deletedLhVolumes: [],
    });
