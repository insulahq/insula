 ...
 *
 * Deletes the external DNS zone for every domain owned by the tenant
 * being deleted. Mirrors the per-domain `deleteZone` call that
 * `domains/service.ts:deleteDomain` already makes when the operator
 * deletes a single domain — but `applyDeleted` on the tenant side
 ...
 *     tenant delete; the orphan zone will be cleaned up on the
...
async function runImpl(ctx: HookCtx): Promise<HookResult> {
  ...
  // List every domain owned by this tenant BEFORE the FK cascade
  // removes the rows. cascades.applyDeleted dispatches the registry
  // BEFORE the `db.delete(tenants)`, so domains rows are still
  // present here.
  const rows = (await ctx.db.select({
    ...
    .from(domains)
    .where(eq(domains.tenantId, ctx.tenantId))) as readonly DnsLite[];

  if (rows.length === 0) {
    return { status: 'noop', detail: 'tenant has no domains' };
  }
