 ...
 * tenant_lifecycle_hook_runs.
 *
 * blocking=continue: an ingress patch failure should not abort the
 * suspend — the tenant row already reflects the new state and the
 ...
 * AND any historical ingress swap if resuming a tenant that was
...
async function runIngressReconcile(ctx: HookCtx): Promise<HookResult> {
  ...
  try {
    ...
    await reconcileIngress(ctx.db, ctx.k8s, ctx.tenantId, ctx.namespace);
    ...
    return {
      ...
      envelope: {
        ...
        detail: err instanceof Error ? err.message : String(err),
        remediation: [
          'Verify ingress_routes for this tenant',
          'Re-run via the lifecycle scheduler retry tick',
        ],
        raw: err instanceof Error ? err.stack ?? err.message : String(err),
      },
