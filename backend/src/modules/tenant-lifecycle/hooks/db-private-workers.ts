...
import { reconcilePrivateWorkersForTenant } from '../../private-workers/reconciler.js';
import {
 ...
 *   - deleted   → hard-delete `private_workers` rows for the tenant.
 ...
 *     down the namespace itself, taking the per-tenant artefacts with
 ...
 * mismatched state vs. the tenant status — better to halt the
  ...
  switch (ctx.transition) {
    case 'suspended': {
      const result = await ctx.db
        ...
        .where(
          and(
            eq(privateWorkers.tenantId, ctx.tenantId),
    ...
    case 'deleted': {
      ...
      await ctx.db
        ...
        .where(eq(privateWorkers.tenantId, ctx.tenantId));
      await tryReconcile(ctx);
...
async function tryReconcile(ctx: HookCtx): Promise<void> {
  try {
    const outcome = await reconcilePrivateWorkersForTenant(
