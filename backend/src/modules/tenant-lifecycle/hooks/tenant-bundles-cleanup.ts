 ...
 *
 * Deletes every off-cluster backup bundle owned by the tenant being
 * deleted. The `backup_jobs` table cascades on `tenants.id` deletion
 ...
 * targetConfigId. Most tenants use 1-2 targets, so the cost is
...
async function runImpl(ctx: HookCtx): Promise<HookResult> {
  ...
  const jobs = (await ctx.db.select({
    ...
    .from(backupJobs)
    .where(eq(backupJobs.tenantId, ctx.tenantId))) as readonly JobLite[];
