...
async function runImpl(ctx: HookCtx): Promise<HookResult> {
  ...
  if (ctx.transition === 'deleted') {
    return { status: 'noop', detail: 'FK cascade owns the tenant row on delete' };
  }
...
export const clientsStatusStampHook: LifecycleHook = {
  ...
  // transition leaves the tenant.status in its pre-transition value.
...
export function registerTenantsStatusStampHook(): void {
