 ...
 *
 * Behaviour:
 *   - archived: enumerate mailboxes by tenantId (BEFORE the
 ...
 *
 * blocking=continue: an impersonator outage SHOULD NOT block tenant
 * archival — the orphan files are small and harmless until cleaned
 * up by a later run. Failed runs land in tenant_lifecycle_hook_runs
...
async function runImpl(ctx: HookCtx): Promise<HookResult> {
  ...
  if (rows.length === 0) {
    return { status: 'noop', detail: `${HOOK_NAME}: no mailboxes owned by tenant` };
  }
