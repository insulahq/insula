/**
 * Full tenant-lifecycle integration test.
 *
...
describe.skipIf(!RUN || !dbAvailable)('tenant lifecycle integration', () => {
  const db = getTestDb();
  let k8s: K8sClients;
  let tenantId: string;
  ...
  // Collect every ad-hoc tenant created mid-test so afterAll can
  ...
  beforeAll(async () => {
    ...
    // Create tenant row directly (skip the provisioning flow — that's
    // tested separately and too slow for this loop).
    tenantId = crypto.randomUUID();
    namespace = `tenant-${uid}-${tenantId.slice(0, 8)}`;
    ...
    await k8s.core.createNamespace({
      body: { metadata: { name: namespace, labels: { 'platform.io/tenant-id': tenantId } } },
  ...
  afterAll(async () => {
    ...
    // state behind. auxCleanup covers any ad-hoc tenants that tests
    ...
    for (const aux of auxCleanup) {
      ...
      await db.delete(tenants).where(eq(tenants.id, aux.id)).catch(() => {});
    }
  ...
  it('seeded tenant starts in active/idle with no mailboxes', async () => {
    const [c] = await db.select().from(tenants).where(eq(tenants.id, tenantId));
    expect(c.status).toBe('active');
    expect(c.storageLifecycleState).toBe('idle');
    const mbs = await db.select().from(mailboxes).where(eq(mailboxes.tenantId, tenantId));
  ...
  it('applySuspended sets status=suspended and disables mailboxes', async () => {
    ...
    await applySuspended({ db, k8s }, tenantId, namespace);

    const [c] = await db.select().from(tenants).where(eq(tenants.id, tenantId));
    expect(c.status).toBe('suspended');
    const [mb] = await db.select().from(mailboxes).where(eq(mailboxes.tenantId, tenantId));
    expect(mb.status).toBe('disabled');
    const [al] = await db.select().from(emailAliases).where(eq(emailAliases.tenantId, tenantId));
    expect(al.enabled).toBe(0);
    const [d] = await db.select().from(domains).where(eq(domains.tenantId, tenantId));
  ...
  it('applyActive reverses the suspend cascade', async () => {
    ...
    await applyActive({ db, k8s }, tenantId, namespace);
  ...
  it('applyArchived deletes mailboxes and aliases', async () => {
    ...
    await applyArchived({ db, k8s }, tenantId, namespace);

    const [c] = await db.select().from(tenants).where(eq(tenants.id, tenantId));
    expect(c.status).toBe('archived');
    const mbs = await db.select().from(mailboxes).where(eq(mailboxes.tenantId, tenantId));
    expect(mbs.length).toBe(0);
    const als = await db.select().from(emailAliases).where(eq(emailAliases.tenantId, tenantId));
    expect(als.length).toBe(0);
  }, 30_000);

  it('applyDeleted removes the tenant row and namespace', async () => {
    const { applyDeleted } = await import('./cascades.js');
    await applyDeleted({ db, k8s }, tenantId, namespace);
  ...
  it('storage_operations enforces state-machine concurrency', async () => {
    // Recreate a fresh tenant to test concurrent-op rejection. Register
    ...
    const tempNs = `tenant-${uid}-concur`;
    auxCleanup.push({ id: tempId, namespace: tempNs });

    await db.insert(tenants).values({
      id: tempId,
      name: `${uid}-concur`,
      primaryEmail: `${uid}-concur@example.test`,
  ...
  it('cleans up storage_operations on test end', async () => {
    ...
    await db.execute(sql`DELETE FROM storage_operations WHERE tenant_id NOT IN (SELECT id FROM tenants)`);
    const orphans = await db.select({ c: sql<number>`count(*)` })
      .from(storageOperations)
      .where(sql`tenant_id NOT IN (SELECT id FROM tenants)`);
