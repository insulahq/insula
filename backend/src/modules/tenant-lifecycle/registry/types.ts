...
/**
 * Five canonical tenant transitions handled by the unified registry.
 * Matches `tenant_lifecycle_transition_kind` in migration 0069.
 *
 * Notes:
...
/**
 * Hook outcome. The dispatcher persists `state` to tenant_lifecycle_hook_runs.
 *
 ...
 * dispatcher's responsibility — hooks never new() their own tenants so
