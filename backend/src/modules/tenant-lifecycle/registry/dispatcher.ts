...
export async function runTransition(
  ...
  // 1. Insert parent row.
  await db.insert(tenantLifecycleTransitions).values({
    id: transitionId,
  ...
  if (opts.triggeredByUserId) {
    try {
      ...
      await startTask(db, {
        ...
        label: toSafeText(`${opts.transition} tenant (${opts.tenantId.slice(0, 8)})`),
        target: {
          type: 'modal',
          modal: 'transition',
          // Mirror the TransitionProgressModal Props shape — tenantId,
      ...
      console.warn(`[tenant-lifecycle] task tracker enroll failed for ${transitionId}: ${msg}`);
    }
  }
  ...
  if (hooks.length === 0) {
    ...
    await db.update(tenantLifecycleTransitions)
      ...
      .where(eq(tenantLifecycleTransitions.id, transitionId));
    return {
      transitionId,
      state: 'completed',
      hooksAttempted: 0,
  ...
  for (let i = 0; i < hooks.length; i++) {
    const h = hooks[i];
    await db.insert(tenantLifecycleHookRuns).values({
      id: randomUUID(),
      transitionId,
      hookName: h.name,
      hookOrder: i,
  ...
  for (const hook of hooks) {
    const runRow = await db.select().from(tenantLifecycleHookRuns)
      .where(eq(tenantLifecycleHookRuns.transitionId, transitionId))
    ...
    await db.update(tenantLifecycleHookRuns)
      ...
      .where(eq(tenantLifecycleHookRuns.id, runRow.id));
  ...
  if (opts.triggeredByUserId) {
    try {
      ...
      const taskStatus =
        ...
        : 'failed';
      await finishByRef(db, 'tenant.transition', transitionId, {
        status: taskStatus,
        text: hooksFailed > 0
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.warn(`[tenant-lifecycle] task tracker finish failed for ${transitionId}: ${msg}`);
    }
  }
