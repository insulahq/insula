...
export async function readClusterState(
  ...
  // (platform.example.test/node-role=server) so workers don't
  ...
  for (const node of nodes.items ?? []) {
    ...
    const isServer = labels['platform.example.test/node-role'] === 'server'
  ...
  for (const node of nodes.items ?? []) {
    if (node.metadata?.labels?.['platform.example.test/node-role'] === 'server') {
...
export const WEBMAIL_ENGINE_DISABLED_ANNOTATION = 'platform.example.test/webmail-engine-disabled';
