...
async function triggerFreshBarmanBackup(
  ...
  const body = {
    ...
    metadata: {
      ...
      namespace,
      labels: {
        'platform.example.test/barman-pre-restore': 'true',
      },
    },
    spec: {
  ...
  const newCluster = {
    ...
    metadata: {
      ...
      labels: {
        ...
        'platform.example.test/barman-restore-source': inputs.sourceClusterName,
        ...(inputs.recoveryTargetTime
          ? { 'platform.example.test/barman-restore-target-time-set': 'true' }
          : { 'platform.example.test/barman-restore-target-time-set': 'false' }),
      },
      annotations: {
        ...(inputs.recoveryTargetTime
          ? { 'platform.example.test/barman-restore-target-time': inputs.recoveryTargetTime }
          : {}),
      },
    },
    spec: {
