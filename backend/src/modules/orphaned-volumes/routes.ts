...
import { detectOrphans, snapshotOrphan, deleteOrphan, findOrphan } from './service.js';

const PV_NAME_PATTERN = /^[a-z0-9]([-a-z0-9]*[a-z0-9])?$/;
const MAX_NAME_LEN = 253;

function validateK8sName(name: string, kind: 'pv' | 'volume'): void {
  if (!name || name.length > MAX_NAME_LEN || !PV_NAME_PATTERN.test(name)) {
    throw new ApiError(
      'INVALID_FIELD_VALUE',
      `Invalid ${kind} name`,
      400,
      { field: kind === 'pv' ? 'pvName' : 'volumeName' },
    );
  }
}
...
export async function orphanedVolumesRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.delete('/admin/orphaned-volumes/:volumeName', {
    ...
    const result = await deleteOrphan(k8s, orphan.pvName, orphan.longhornVolumeName ?? volumeName);
    return success(result);
  });
}
