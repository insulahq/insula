...
export async function createTenant(db: Database, input: CreateTenantInput, createdBy: string) {
  ...
  await db.insert(tenants).values({
    ...
    billingStreetAddress: input.billing_address.street_address,
    billingPostalAddress: input.billing_address.postal_address,
    billingCity: input.billing_address.city,
    billingCountry: input.billing_address.country,
    status: 'pending',
