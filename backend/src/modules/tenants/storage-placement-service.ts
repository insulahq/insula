 ...
 *   for a Local-tier tenant, we pick the host-tenant-workloads node
...
import { tenants, deployments, clusterNodes, hostingPlans } from '../../db/schema.js';
...
export interface NodeCapacityForPick {
  ...
  /** can this node host tenant workloads? */
...
/**
 * Auto-pick a worker node for a Local-tier tenant at provisioning.
 * Strategy: among host-tenant-workloads nodes, pick the one with the
 ...
 * node exists — caller falls back to leaving nodeName null
...
export async function autoPickWorkerNode(
  ...
  try {
    ...
    const tenantCapable = new Map(platformNodes.map((n) => [n.name, n.canHostTenantWorkloads]));
 ...
 *   3. Persist tenants.storage_tier.
 *
 * `previousTier` is passed in by the caller (typically `updateTenant`)
 ...
 * ApiError on bad inputs (unknown tenant, invalid tier).
 */
export async function applyTenantTier(
  ...
  if (previousTier !== newTier) {
    ...
    try {
      await patchTenantVolumeReplicas(k8s, tenant.kubernetesNamespace, target);
  ...
  // Patch every Deployment in the namespace owned by this tenant. We
  ...
  let patchedCount = 0;
  if (k8s && tenantDeploys.length > 0 && tenant.nodeName) {
    const ns = tenant.kubernetesNamespace;
    const nodeSelector = newTier === 'ha' ? null : { 'kubernetes.io/hostname': tenant.nodeName };
  ...
  // this, a local-tier flip pins tenant to tenant.nodeName but
  // FM keeps running on whatever node it was originally scheduled on,
  // and the RWO `tenant-storage` PVC mount races into a Multi-Attach
  ...
  //   • local tier  → nodeSelector pins FM to tenant.nodeName
  ...
  if (k8s && tenant.nodeName) {
    const ns = tenant.kubernetesNamespace;
    const fmNodeSelector = newTier === 'local'
      ? { 'kubernetes.io/hostname': tenant.nodeName }
      : null;
    try {
      await k8s.apps.patchNamespacedDeployment({
        name: 'file-manager',
...
/**
 * Resolve a "free vs total" capacity summary for every host-tenant-
...
export async function listWorkerCandidatesWithUsage(
  ...
  const platformNodes = await db.select().from(clusterNodes);
  const candidates = platformNodes.filter((n) => n.canHostTenantWorkloads);
