/**
 * Capacity preflight for tenant create.
 *
 * Phase G: before we accept a new tenant and start provisioning, check
 * whether Longhorn actually has room for their PVCs. The platform was
 * happily accepting tenants onto a cluster that couldn't physically
 ...
 * the operator has already told the new tenant their account is ready.
 *
 * What this checks:
 ...
 * an HA-tier tenant because there aren't enough tenant-capable nodes.
 * Used by both POST /tenants (create with storage_tier=ha) and
 * PATCH /tenants (live tier flip local→ha) so an operator can't
...
export async function assertHaTierFeasible(db: Database): Promise<void> {
  const eligible = await db
    ...
    .from(clusterNodes)
    .where(eq(clusterNodes.canHostTenantWorkloads, true));
  ...
  throw new ApiError(
    ...
    409,
    {
      operatorError: {
        ...
        remediation: [
          `Add worker nodes (canHostTenantWorkloads=true) until at least ${HA_MIN_TENANT_NODES} are present.`,
...
export async function checkProvisioningCapacity(
  ...
  // No K8s tenant (unit tests, broken kubeconfig) — skip preflight,
  // don't block tenant creation. The original failure-on-provision
  ...
  if (!k8s) {
    return {
      ...
      nodes: [],
      fittingNodes: 0,
      reason: 'k8s tenant unavailable; preflight skipped',
    };
  }
  ...
  try {
    ...
    // canHostTenantWorkloads — Longhorn nodes are visible even on
    ...
    const tenantCapable = new Set(
      platformNodes.filter((n) => n.canHostTenantWorkloads).map((n) => n.name),
    );
