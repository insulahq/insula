...
describe('deriveRegionId', () => {
  it('slugifies a domain by replacing dots with dashes', () => {
    expect(deriveRegionId('staging.example.test')).toBe('staging-example-test');
    expect(deriveRegionId('example.test')).toBe('example-test');
    expect(deriveRegionId('testing.example.test')).toBe('testing-example-test');
  });

  it('respects an operator override unchanged', () => {
    expect(deriveRegionId('staging.example.test', 'eu-fsn1')).toBe('eu-fsn1');
    expect(deriveRegionId('staging.example.test', '')).toBe('staging-example-test');
  });
