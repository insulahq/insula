...
describe('cluster-scoped-refs-cleanup hook', () => {
  ...
  it('deletes CRBs whose subjects reference the namespace', async () => {
    const { ctx, log } = makeCtx({
      crbs: [
        { name: 'sftp-acme', subjects: [{ namespace: 'client-acme' }] },
  ...
  it('deletes cross-ns NetworkPolicies referencing the namespace', async () => {
    const { ctx, log } = makeCtx({
      netpols: [
        { ns: 'app-shared', name: 'allow-acme', matchNs: 'client-acme' },
        { ns: 'client-acme', name: 'self-ref', matchNs: 'client-acme' }, // own ns — skipped
  ...
  it('returns retry envelope when one delete throws', async () => {
    ...
    const { ctx } = makeCtx({
      crbs: [{ name: 'sftp-acme', subjects: [{ namespace: 'client-acme' }] }],
      deleteThrows: throws,
    });
    const r = await clusterScopedRefsCleanupHook.run(ctx);
  ...
  it('treats 404 on delete as success (idempotent)', async () => {
    ...
    const { ctx } = makeCtx({
      crbs: [{ name: 'gone-crb', subjects: [{ namespace: 'client-acme' }] }],
      deleteThrows: throws,
    });
    const r = await clusterScopedRefsCleanupHook.run(ctx);
