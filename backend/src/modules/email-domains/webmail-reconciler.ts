...
// The per-tenant engine-routing reconciler lived here.
//
// It re-targeted a per-domain ExternalName Service at whichever webmail engine
// was active, because `ensureWebmailIngress` used to reverse-proxy the tenant
// hostname at that Service — so flipping Roundcube/Bulwark left every tenant
// route pointing at a Deployment scaled to zero.
//
// The tenant hostname now answers with a 302 to the platform webmail origin,
// which is engine-agnostic: there is no per-tenant object that names an engine,
// so there is nothing left to re-target. Keeping the reconciler would have been
// actively harmful — it treats a missing ExternalName Service as drift and
// rebuilds, so against the redirect it would re-bootstrap every domain on
// every tick, forever.
//
// Certificate reconciliation below is unaffected and still required: the
// handshake for `webmail.<domain>` completes before the redirect is sent.
