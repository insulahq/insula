    ...
    const observed = projectNode({
      metadata: {
        ...
        labels: {
          ...
          'platform.example.test/host-tenant-workloads': 'true',
        },
      },
    });
    expect(observed.role).toBe('server');
...
describe('applyNewServerDefault', () => {
  ...
  it('stamps host-tenant-workloads=true and removes server-only taint when default=true', async () => {
    ...
    expect(body.metadata.labels['platform.example.test/host-tenant-workloads']).toBe('true');
    expect(body.spec.taints.find((t) => t.key === 'platform.example.test/server-only')).toBeUndefined();
  });
  ...
  it('stamps host-tenant-workloads=false and applies server-only NoSchedule taint when default=false', async () => {
    ...
    expect(body.metadata.labels['platform.example.test/host-tenant-workloads']).toBe('false');
    const ours = body.spec.taints.find((t) => t.key === 'platform.example.test/server-only');
    expect(ours).toEqual({ key: 'platform.example.test/server-only', value: 'true', effect: 'NoSchedule' });
  });
  ...
  it('preserves operator-set taints with other keys when default=false', async () => {
    ...
    expect(body.spec.taints.map((t) => t.key)).toContain('platform.example.test/server-only');
  });
});
...
describe('projectNode + applyNewServerDefault contract', () => {
  ...
  it('an unlabeled server projects to canHostTenantWorkloads=false (the trigger)', () => {
    ...
    expect(labels['platform.example.test/host-tenant-workloads']).toBeUndefined();
  });
});
