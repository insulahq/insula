...
export const TERMINAL_POD_LABEL = 'platform.example.test/node-terminal';
export const TERMINAL_SESSION_LABEL = 'platform.example.test/session-id';
export const TERMINAL_TARGET_NODE_LABEL = 'platform.example.test/target-node';
 ...
 *      can list `platform.example.test/node-terminal=true` and
...
export function buildTerminalPodSpec(input: BuildTerminalPodSpecInput): k8s.V1Pod {
  ...
  return {
    ...
    metadata: {
      ...
      annotations: {
        'platform.example.test/created-at': new Date().toISOString(),
      },
