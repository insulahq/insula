...
describe('computeDefaults', () => {
  it('derives standard subdomains from an apex', () => {
    expect(computeDefaults('staging.example.test')).toEqual({
      ...
      stalwartAdminUrl: 'https://mail-admin.staging.example.test/',
...
describe('getPlatformUrls', () => {
  it('returns stored values when set', async () => {
    const { db } = createMockDb({
      ingress_base_domain: 'staging.example.test',
  ...
  it('falls back to apex-derived defaults when DB rows are missing', async () => {
    ...
    expect(result.stalwartAdminUrl.value).toBe('https://mail-admin.staging.example.test/');
...
describe('updatePlatformUrls', () => {
  it('writes the four keys to the DB', async () => {
    const { db, store } = createMockDb({ ingress_base_domain: 'staging.example.test' });
