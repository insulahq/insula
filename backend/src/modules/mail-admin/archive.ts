...
async function createArchiveJob(
  ...
  const toolsImage =
    ...
    'ghcr.io/insulahq/hosting-platform/mail-backup-tools:latest';
  const stalwartImage =
  ...
  const body: Record<string, unknown> = {
    ...
    metadata: {
      ...
      labels: {
        ...
        'platform.example.test/archive-mode': mode,
      },
      annotations: {
        'platform.example.test/started-by': deps.userId ?? 'system',
      },
    },
    spec: {
      backoffLimit: 0,
...
async function createArchiveJobNoDowntime(
  ...
  const rocksdbSecondaryImage =
    ...
    'ghcr.io/insulahq/hosting-platform/rocksdb-secondary-checkpoint:latest';
  ...
  const body: Record<string, unknown> = {
    ...
    metadata: {
      ...
      labels: {
        'app.kubernetes.io/component': 'stalwart-archive',
        'app.kubernetes.io/managed-by': 'platform-api',
        'platform.example.test/archive-mode': 'no_downtime',
      },
      annotations: {
        'platform.example.test/started-by': deps.userId ?? 'system',
      },
