...
describe('listServerNodeIps', () => {
    ...
    const core = makeCore([
      {
        metadata: {
          name: 'w1',
          labels: { 'platform.example.test/node-role': 'worker' },
        },
    ...
    const core = makeCore([
      {
        metadata: {
          name: 's3',
          labels: { 'platform.example.test/node-role': 'server', 'kubernetes.io/hostname': 's3' },
        },
      ...
      },
      {
        metadata: {
          name: 's1',
          labels: { 'platform.example.test/node-role': 'server', 'kubernetes.io/hostname': 's1' },
        },
  ...
  it('skips nodes without an InternalIP', async () => {
    const core = makeCore([
      {
        metadata: { name: 's1', labels: { 'platform.example.test/node-role': 'server' } },
  ...
  it('rejects bogus loopback/unspecified InternalIPs (defense vs misconfigured kubelet)', async () => {
    const core = makeCore(
      ['127.0.0.1', '0.0.0.0', '::1'].map((ip, i) => ({
        metadata: {
          name: `n${i}`,
          labels: { 'platform.example.test/node-role': 'server', 'kubernetes.io/hostname': `n${i}` },
        },
    ...
    const core = makeCore([
      {
        metadata: {
          name: 'duplicate',
          labels: { 'platform.example.test/node-role': 'server', 'kubernetes.io/hostname': 'dup' },
        },
