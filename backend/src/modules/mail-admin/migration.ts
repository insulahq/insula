...
async function runMigrationStateMachine(
  ...
  try {
    await (core as unknown as {
      ...
      { metadata: { annotations: { 'platform.example.test/delete-after': new Date(Date.now() + 86400000).toISOString() } } },
      undefined, undefined, undefined, undefined,
...
async function spawnRsyncJob(
  ...
  await (batch as unknown as {
    ...
    metadata: {
      ...
      annotations: {
        'platform.example.test/migration-source-node': sourceNode,
        'platform.example.test/migration-target-node': targetNode,
        'platform.example.test/migration-target-pvc': targetPvcName,
        'platform.example.test/ttl-exempt': 'false',
      },
    },
    spec: {
      backoffLimit: 2,
