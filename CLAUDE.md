...
**Security / Firewall / Node Hardening page**: `/settings/security-hardening` (super_admin) — read-mostly observability across cluster nodes. SSH posture, mesh detection (NetBird/Tailscale/WireGuard), per-node CIS-style checks, firewall posture, audit-log gap detector, TLS-cert expiry, backup-target encryption + freshness, Calico WG verification, reserved-hostname collision feed (ADR-040). Driven by `security-probe` DaemonSet (read-only host mounts, drops ALL caps) writing one ConfigMap per node. Destructive ops surface as guided runbooks; SSH lockdown via `bootstrap.sh --ssh-via-mesh <iface>` (opt-in; renders `iif "<iface>" tcp dport 22 accept` + `trusted_ranges_v{4,6}` saddr fallback; persists to `/etc/hosting-platform/firewall.conf`). Trivy CVE scanning intentionally deferred. Operator runbook: [docs/04-deployment/SECURITY_HARDENING.md](docs/04-deployment/SECURITY_HARDENING.md). Phase plan + suggestions: [docs/04-deployment/SECURITY_HARDENING_ROADMAP.md](docs/04-deployment/SECURITY_HARDENING_ROADMAP.md). CI guards `scripts/ci-firewall-check.sh` (extended invariant 3) + `scripts/test-ssh-via-mesh.sh`.
