...
#
# Three pieces, each verified individually INSUFFICIENT on the DEV cluster:
#
#   1. kubelet KubeletConfiguration drop-in. shutdownGracePeriod has no
#      command-line flag (config-file-only), so `--kubelet-arg` cannot express
#      it. k3s merges /var/lib/rancher/k3s/agent/etc/kubelet.conf.d/*.conf over
#      its generated 00-k3s-defaults.conf and rewrites only its own 00- file.
#
