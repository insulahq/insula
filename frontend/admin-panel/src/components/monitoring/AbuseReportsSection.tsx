/**
 * Abuse reports (ARF, RFC 5965).
 *
 * Sibling of DmarcSection, deliberately shaped differently. DMARC aggregates
 * because one record means nothing — the signal is the trend. A complaint is
 * the opposite: one of them IS the event, and rolling them into a per-domain
 * count would hide the two lines an operator actually acts on, which message
 * and who complained. So this is a list, newest first, with the detail inline.
 *
 * Unattributed complaints are shown, not filtered. A report naming a domain
 * this platform does not host is usually somebody spoofing one of ours, and
 * hiding it would make the abuse desk look quiet at the worst moment.
 */
import { useState } from 'react';
import { useQuery } from '@tanstack/react-query';
import { ShieldAlert, Inbox, Loader2, ChevronRight, ChevronDown, Info } from 'lucide-react';
import { apiFetch } from '@/lib/api-client';
import type { AbuseReportsOverview, AbuseReport, AbuseFeedbackType } from '@insula/api-contracts';

function useAbuseReports(windowDays: number) {
  return useQuery({
    queryKey: ['mail', 'abuse-reports', windowDays],
    queryFn: () => apiFetch<{ data: AbuseReportsOverview }>(
      `/api/v1/admin/mail/abuse-reports?windowDays=${windowDays}`,
    ),
    staleTime: 60_000,
  });
}

const TYPE_STYLE: Record<AbuseFeedbackType, { label: string; cls: string }> = {
  abuse: {
    label: 'Abuse',
    cls: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300',
  },
  fraud: {
    label: 'Fraud',
    cls: 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300',
  },
  virus: {
    label: 'Virus',
    cls: 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300',
  },
  other: {
    label: 'Other',
    cls: 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300',
  },
};

function formatWhen(iso: string): string {
  const d = new Date(iso);
  return Number.isNaN(d.getTime()) ? iso : d.toLocaleString();
}

function Row({ report }: { report: AbuseReport }) {
  const [open, setOpen] = useState(false);
  const style = TYPE_STYLE[report.feedbackType] ?? TYPE_STYLE.other;
  return (
    <>
      <tr
        className="cursor-pointer border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/40"
        onClick={() => setOpen((v) => !v)}
        data-testid={`abuse-row-${report.id}`}
      >
        <td className="px-3 py-2">
          <span className="inline-flex items-center gap-1 text-gray-500 dark:text-gray-400">
            {open ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
            <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${style.cls}`}>{style.label}</span>
          </span>
        </td>
        <td className="px-3 py-2 font-mono text-xs text-gray-900 dark:text-gray-100">
          {report.domain ?? <span className="italic text-gray-400">not stated</span>}
        </td>
        <td className="px-3 py-2 text-sm text-gray-700 dark:text-gray-300">
          {report.tenantName ?? (
            <span
              className="text-amber-600 dark:text-amber-400"
              title="The reported domain is not an email domain on this platform — often a spoof of one."
            >
              unattributed
            </span>
          )}
        </td>
        <td className="px-3 py-2 text-sm tabular-nums text-gray-700 dark:text-gray-300">
          {report.incidents > 1 ? `${report.incidents}×` : '1'}
        </td>
        <td className="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">{formatWhen(report.receivedAt)}</td>
      </tr>
      {open && (
        <tr className="border-t border-gray-100 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/40">
          <td colSpan={5} className="px-6 py-3">
            <dl className="grid grid-cols-1 gap-x-8 gap-y-2 text-xs sm:grid-cols-2">
              <Detail label="Reported by" value={report.reporter} />
              <Detail label="Reporting MTA" value={report.reportingMta} />
              <Detail label="Original sender" value={report.originalMailFrom} mono />
              <Detail label="Original recipient" value={report.originalRcptTo} mono />
              <Detail label="Source IP" value={report.sourceIp} mono />
              <Detail label="Subject" value={report.subject} />
            </dl>
          </td>
        </tr>
      )}
    </>
  );
}

function Detail({ label, value, mono }: { label: string; value: string | null; mono?: boolean }) {
  return (
    <div className="flex gap-2">
      <dt className="shrink-0 font-medium text-gray-500 dark:text-gray-400">{label}:</dt>
      <dd className={`break-all text-gray-800 dark:text-gray-200 ${mono ? 'font-mono' : ''}`}>
        {value ?? <span className="italic text-gray-400">not stated</span>}
      </dd>
    </div>
  );
}

export default function AbuseReportsSection() {
  const [windowDays, setWindowDays] = useState(30);
  const { data, isLoading, isError, error } = useAbuseReports(windowDays);
  const overview = data?.data;

  return (
    <section
      className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-5 space-y-4"
      data-testid="abuse-reports-section"
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <ShieldAlert size={20} className="text-gray-700 dark:text-gray-300" />
          <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">Abuse reports</h3>
        </div>
        <label className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
          Window
          <select
            value={windowDays}
            onChange={(e) => setWindowDays(Number(e.target.value))}
            className="rounded-lg border border-gray-300 dark:border-gray-600 px-2 py-1 text-xs text-gray-900 dark:bg-gray-700 dark:text-gray-100"
            data-testid="abuse-window-select"
          >
            <option value={7}>7 days</option>
            <option value={30}>30 days</option>
            <option value={90}>90 days</option>
          </select>
        </label>
      </div>

      <p className="text-xs text-gray-500 dark:text-gray-400">
        Complaints other operators sent about mail from this platform, parsed from{' '}
        <abbr title="Abuse Reporting Format, RFC 5965">ARF</abbr> reports delivered to{' '}
        <code>{overview?.intakeLocalPart ?? 'abuse'}@</code>. Every one also raises an admin
        notification when it arrives.
      </p>

      {isLoading && (
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
          <Loader2 size={16} className="animate-spin" /> Loading reports…
        </div>
      )}

      {isError && (
        <p className="text-sm text-red-600 dark:text-red-400">
          Could not load abuse reports: {(error as Error)?.message ?? 'unknown error'}
        </p>
      )}

      {overview && overview.reports.length === 0 && (
        <div className="flex items-start gap-2 rounded-lg bg-gray-50 dark:bg-gray-900/40 p-3 text-sm text-gray-600 dark:text-gray-400">
          <Inbox size={16} className="mt-0.5 shrink-0" />
          <span>
            No abuse reports in the last {overview.windowDays} days. Complaints arrive
            unsolicited, so an empty list is the normal state — not a sign that ingestion is
            broken.
          </span>
        </div>
      )}

      {overview && overview.reports.length > 0 && (
        <>
          <div className="overflow-x-auto">
            <table className="min-w-full text-left">
              <thead>
                <tr className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
                  <th className="px-3 py-2 font-medium">Type</th>
                  <th className="px-3 py-2 font-medium">Reported domain</th>
                  <th className="px-3 py-2 font-medium">Tenant</th>
                  <th className="px-3 py-2 font-medium">Incidents</th>
                  <th className="px-3 py-2 font-medium">Received</th>
                </tr>
              </thead>
              <tbody>
                {overview.reports.map((r) => <Row key={r.id} report={r} />)}
              </tbody>
            </table>
          </div>
          {overview.total > overview.reports.length && (
            <p className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
              <Info size={13} />
              Showing the {overview.reports.length} most recent of {overview.total} in this window.
            </p>
          )}
        </>
      )}
    </section>
  );
}
