...
/**
 * Domains & SMTP Relays section. Self-contained — owns the two-tab
 * switcher between the per-tenant email-domains table and the
 * SMTP relay registry. Lifted out of EmailManagement.tsx during
 * the IA split so the EmailDomainsPage can render it directly under
 * the shared header.
 */
export default function EmailDomainsSection() {
  const [tab, setTab] = useState<DomainsTab>('domains');
...
function EmailDomainsTable({
  domains,
  isLoading,
}: {
  readonly domains: readonly EmailDomainRow[];
  readonly isLoading: boolean;
}) {
  const { sortedData, sortKey, sortDirection, onSort } = useSortable(domains, 'domainName');
          ...
          {domains.length === 0 && (
            <tr>
              <td colSpan={7} className="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                No email-enabled domains yet.
              </td>
            </tr>
          )}
        </tbody>
      </table>
      {dkimDomain && <DkimStatusModal domain={dkimDomain} onClose={() => setDkimDomain(null)} />}
    </div>
  );
}

function EmailDomainRowView({
  domain: d,
  onOpenDkim,
}: {
  readonly domain: EmailDomainRow;
  readonly onOpenDkim: () => void;
}) {
  const updateDomain = useUpdateEmailDomain(d.tenantId);
  const webmailOn =
    d.webmailEnabled !== 0 && d.webmailEnabled !== undefined ? d.webmailEnabled === 1 : true;
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {!isLoading && status?.zoneFileAvailable && status.selectors.length === 0 && (
            <div className="rounded-lg border border-dashed border-amber-200 dark:border-amber-700 p-6 text-center text-sm text-amber-700 dark:text-amber-300">
              No DKIM selector records found in the Stalwart zone file. DKIM may not be configured for this domain yet — check the Stalwart admin UI.
            </div>
          )}
          ...
          {!isLoading && status?.zoneFileAvailable && status.selectors.length > 0 && (
            <div className="space-y-3">
              ...
              {status.selectors.map((sel) => <DkimSelectorCard key={sel.name} selector={sel} />)}
            </div>
          )}
        </div>
      </div>
...
function SmtpRelaysSection() {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    if (provider === 'mailgun') {
      input = {
        ...
        smtp_host: form.smtp_host || 'smtp.eu.mailgun.org',
  ...
  const handleTest = async (id: string) => {
    try {
      ...
      setTestResult({ id, status: 'error', message: 'Test failed' });
    }
  };
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        ...
        >
          ...
          {provider === 'postmark' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">API Key / Server Token</label>
              <input
                className={INPUT_CLASS + ' mt-1 font-mono'}
                value={form.api_key}
          ...
          {provider === 'direct' && (
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Direct delivery — Stalwart sends email without an external relay. Requires proper IP reputation and PTR record.
            </p>
          )}
          <div className="flex justify-end">
            <button
        ...
        relays.map((r) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {r.isDefault ? (
                    <span className="rounded bg-brand-100 dark:bg-brand-900/20 px-2 py-0.5 text-xs font-medium text-brand-700 dark:text-brand-300">
                      Default
                    </span>
                  ) : null}
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {r.smtpHost ? `${r.smtpHost}:${r.smtpPort}` : 'Direct delivery'}
                  {r.region ? ` (${r.region})` : ''}
                </p>
              </div>
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button
                type="button"
                onClick={() => deleteRelay.mutate(r.id)}
