...
/**
 * Platform → Limits & Regional — API rate limit, default timezone, currency.
 * Sends a partial PATCH on save.
 */
  ...
  if (isError) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-sm text-red-700 dark:text-red-400">
        <AlertCircle size={16} />
        <span>Failed to load limits: {error?.message ?? 'Unknown error'}</span>
      </div>
    );
  }
    ...
    <div className="space-y-6" data-testid="limits-page">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">API Rate Limit</label>
