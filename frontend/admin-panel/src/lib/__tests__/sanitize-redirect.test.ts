...
describe('sanitizeRedirect', () => {
  const ADMIN_ORIGIN = 'https://admin.staging.example.test';
  const APEX = 'staging.example.test';
  ...
  it('passes through an absolute URL on the same admin origin', () => {
    const rd = 'https://admin.staging.example.test/settings/storage';
    expect(sanitizeRedirect(rd, ADMIN_ORIGIN, APEX)).toBe(rd);
  });

  it('passes through an absolute URL on a sibling admin-only subdomain', () => {
    // longhorn.staging.example.test shares the apex — trust it.
    const rd = 'https://longhorn.staging.example.test/';
    expect(sanitizeRedirect(rd, ADMIN_ORIGIN, APEX)).toBe(rd);
  });
  ...
  it('rejects absolute URLs that spoof the apex as a subpath', () => {
    // https://evil.com/staging.example.test/ — host is evil.com.
    expect(sanitizeRedirect('https://evil.com/staging.example.test/', ADMIN_ORIGIN, APEX)).toBe('/');
  });

  it('rejects absolute URLs that embed the apex as a sub-domain suffix', () => {
    // staging.example.test.evil.com — attacker-controlled host that
    // ends in ".evil.com", and happens to start with the apex. Must not
    // match the `endsWith('.' + apex)` predicate naively.
    expect(sanitizeRedirect('https://staging.example.test.evil.com/', ADMIN_ORIGIN, APEX)).toBe('/');
  });
  ...
  it('preserves hash and query on a same-apex absolute URL', () => {
    const rd = 'https://longhorn.staging.example.test/#/backup?env=staging';
    expect(sanitizeRedirect(rd, ADMIN_ORIGIN, APEX)).toBe(rd);
  });

  it('accepts the apex itself (e.g. a root-apex portal) — treated as same-apex', () => {
    const rd = 'https://staging.example.test/';
    expect(sanitizeRedirect(rd, ADMIN_ORIGIN, APEX)).toBe(rd);
  });
  ...
  it('resolves a bare path ("dashboard") against the admin origin — same-apex, safe', () => {
    // WHATWG URL resolution gives https://admin.staging.example.test/dashboard
    // which passes the apex gate. Accepted.
    expect(sanitizeRedirect('dashboard', ADMIN_ORIGIN, APEX)).toBe(
      'https://admin.staging.example.test/dashboard',
    );
  });
