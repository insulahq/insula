...
const OK_USER = { id: 'u1', email: 'a@example.test', fullName: 'A', role: 'super_admin' };
...
describe('attemptRefresh(): which refresh failures end the session', () => {
  it('does not show the expired overlay when the refresh is rate-limited', async () => {
    ...
    await expect(apiFetch('/api/v1/tenants')).rejects.toBeInstanceOf(ApiError);

    // The session must survive: a throttled refresh says nothing about
    // whether the refresh token is still good.
    expect(document.getElementById('token-expired-overlay')).toBeNull();
    expect(localStorage.getItem('auth_refresh_token')).toBe('valid-refresh');
  });
  ...
  it('DOES end the session when the refresh token is genuinely rejected', async () => {
    ...
    expect(document.getElementById('token-expired-overlay')).not.toBeNull();
  });
  ...
  it('retries the original request after a successful refresh', async () => {
    ...
    await expect(apiFetch('/api/v1/tenants')).resolves.toEqual({ data: 'retried' });
  ...
  it('surfaces the rate-limit error to the caller instead of swallowing it', async () => {
    ...
    const err = await apiFetch('/api/v1/tenants').catch((e: unknown) => e);
