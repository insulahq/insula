...
const mockUser = { id: 'u1', email: 'u@c1.com', fullName: 'Me', role: 'tenant_admin' };
...
vi.mock('../hooks/use-tenant-context', () => ({
  useTenantContext: () => ({ tenantId: 'c1', tenantName: 'Test', isLoading: false }),
}));
...
describe('useCanManage hook', () => {
  ...
  it('returns false for tenant_user (read-only)', () => {
...
describe('Page gating — Domains', () => {
  ...
  it('shows Add Domain button for tenant_admin', async () => {
    mockUser.role = 'tenant_admin';
  ...
  it('hides Add Domain button and shows read-only notice for tenant_user', async () => {
    mockUser.role = 'tenant_user';
    const { default: Domains } = await import('../pages/Domains');
    render(wrap(<Domains />));
...
describe('Page gating — CronJobs', () => {
  ...
  it('shows Add Cron Job button for tenant_admin', async () => {
    mockUser.role = 'tenant_admin';
  ...
  it('hides Add Cron Job button and shows notice for tenant_user', async () => {
    mockUser.role = 'tenant_user';
    const { default: CronJobs } = await import('../pages/CronJobs');
    render(wrap(<CronJobs />));
...
describe('Page gating — SshKeys', () => {
  ...
  it('shows Add SSH Key button for tenant_admin', async () => {
    mockUser.role = 'tenant_admin';
  ...
  it('hides Add SSH Key button and shows notice for tenant_user', async () => {
    mockUser.role = 'tenant_user';
    const { default: SshKeys } = await import('../pages/SshKeys');
    render(wrap(<SshKeys />));
