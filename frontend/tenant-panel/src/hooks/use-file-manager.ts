...
export function useFileManagerStatus() {
  ...
  return useQuery({
    queryKey: ['file-manager-status', tenantId],
...
export function useDirectoryListing(path: string, enabled = true) {
  const { tenantId } = useTenantContext();
  return useQuery({
    queryKey: ['files', tenantId, path],
    queryFn: () => apiFetch<{ data: DirectoryListing }>(`/api/v1/tenants/${tenantId}/files?path=${encodeURIComponent(path)}`),
    select: (res) => res.data,
    enabled,
  });
}
...
export function useFileContent(path: string, enabled = true) {
  const { tenantId } = useTenantContext();
  return useQuery({
    queryKey: ['file-content', tenantId, path],
    queryFn: () => apiFetch<{ data: FileContent }>(`/api/v1/tenants/${tenantId}/files/read?path=${encodeURIComponent(path)}`),
    select: (res) => res.data,
    enabled,
  });
}
...
export function useWriteFile() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['file-content', tenantId] });
    },
  });
}
...
export function useAuthenticatedBlobUrl(path: string, enabled = true) {
  const { tenantId } = useTenantContext();
  return useQuery({
    queryKey: ['file-blob', tenantId, path],
    queryFn: async () => {
      const url = `${API_BASE}/api/v1/tenants/${tenantId}/files/download?path=${encodeURIComponent(path)}`;
...
export function useDownloadFile() {
  ...
  return useCallback((path: string) => {
    const token = localStorage.getItem('auth_token') ?? '';
    const url = `${API_BASE}/api/v1/tenants/${tenantId}/files/download?path=${encodeURIComponent(path)}&token=${encodeURIComponent(token)}`;
...
export function useUploadFiles() {
  ...
  const uploadFile = useCallback((file: File, targetDir: string) => {
    ...
    if (file.size > CHUNK_THRESHOLD) {
      uploadFileChunked(tenantId!, qc, file, filePath, setUploads, setVisible);
      return;
    }

    const url = `${API_BASE}/api/v1/tenants/${tenantId}/files/upload-raw?path=${encodeURIComponent(filePath)}`;
...
export function useDiskUsage() {
  const { tenantId } = useTenantContext();
  return useQuery({
    queryKey: ['disk-usage', tenantId],
    queryFn: () => apiFetch<{ data: DiskUsage }>(
      `/api/v1/tenants/${tenantId}/files/disk-usage`
    ),
