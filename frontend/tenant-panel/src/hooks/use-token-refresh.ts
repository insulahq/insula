 ...
 *
 * Impersonated sessions are deliberately NOT covered: `POST
...
export function useTokenRefresh() {
  const maybeRefresh = useCallback(async () => {
    ...
    // No refresh token: either signed out, or an impersonation session (which
    // is issued without one by design). Nothing to rotate either way.
