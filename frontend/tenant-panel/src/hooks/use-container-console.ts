...
export function useDeploymentComponents(deploymentId: string) {
  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/v1/tenants/${tenantId}/deployments/${deploymentId}/components`, {
...
export function useLogStream(
  ...
  const connect = useCallback(() => {
    if (!tenantId || !deploymentId || !enabled) return;
    ...
    const url = buildWsUrl(`/api/v1/tenants/${tenantId}/deployments/${deploymentId}/logs/stream?${params}`);
  ...
  }, [tenantId, deploymentId, component, tailLines, enabled]);
...
export function useTerminal(
  ...
  const connect = useCallback(
    (onData: (data: string) => void) => {
      ...
      const url = buildWsUrl(`/api/v1/tenants/${tenantId}/deployments/${deploymentId}/terminal?${params}`);
      const ws = new WebSocket(url);
      wsRef.current = ws;
    ...
    [tenantId, deploymentId, component, shell],
  );
