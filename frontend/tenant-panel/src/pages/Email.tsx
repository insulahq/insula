...
export default function Email() {
  ...
  // is missing or refers to a domain the tenant no longer has, fall
  ...
  // Review MEDIUM-3: if the tenant has NO email domains at all,
...
//
// Shows the list of a tenant's domains that do NOT yet have email
...
// newly-added domains can be enabled even after the tenant already
...
function EnableEmailCard({
  ...
  // If the domains list is still loading AND the tenant has no
  // email-enabled domains yet, show a spinner inside the card so the
  // user sees "something is happening". If the tenant already has
...
function DisableEmailCard({
  ...
  const preview = useEmailDomainDisablePreview(tenantId, domain.domainId, open);
  const disable = useDisableEmailDomain(tenantId);
...
function MailboxesTab({
  ...
  const { data: res, isLoading } = useMailboxes(tenantId, emailDomain.id);
  const deleteMailbox = useDeleteMailbox(tenantId);
  ...
  const createMailbox = useCreateMailbox(tenantId, emailDomain.id);
  ...
  return (
    <div className="space-y-4">
      <MailboxUsageBar tenantId={tenantId} />
...
// ─── Edit Mailbox Modal ────────────────────────────────────────────────────
//
// Lets a tenant_admin change the mailbox display name, password,
...
function DnsRecordsCard({
  ...
  const { data: res, isLoading, error } = useEmailDomainDnsRecords(tenantId, domain.domainId);
  const dns = res?.data;
...
function DkimKeysCard({
  ...
  const { data: keysRes, isLoading } = useDkimKeys(tenantId, domain.domainId);
  const rotate = useRotateDkimKey(tenantId, domain.domainId);
  const activate = useActivateDkimKey(tenantId, domain.domainId);
...
function ImapSyncPanel({
  ...
  const purge = usePurgeImapSyncJob(tenantId);
  const resync = useResyncImapSyncJob(tenantId);
  const update = useUpdateImapSyncJob(tenantId);
  const [showForm, setShowForm] = useState(false);
