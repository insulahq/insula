...
function CreateModal({ tenantId, onClose, onCreated }: CreateModalProps) {
  const create = useCreatePrivateWorker(tenantId);
...
export default function PrivateWorkers() {
  ...
  const { data, isLoading, error } = usePrivateWorkers(tenantId ?? undefined);
