...
function StatusBadge({ status }: { readonly status: string }) {
  const colorMap: Record<string, string> = {
    ...
    verified: 'bg-green-50 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-700',
    unverified: 'bg-yellow-50 text-yellow-700 border-yellow-200 dark:bg-yellow-900/40 dark:text-yellow-300 dark:border-yellow-700',
    ...
    deleted: 'bg-red-50 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-700',
...
export default function Domains() {
  ...
  const { data, isLoading, isError, error } = useDomains(tenantId ?? undefined);
  ...
  return (
    <div className="space-y-6">
      ...
      {showAddModal && (
        <AddDomainModal tenantId={tenantId!} onClose={() => setShowAddModal(false)} />
      )}
    </div>
  );
}
...
function AddDomainModal({ tenantId, onClose }: { readonly tenantId: string; readonly onClose: () => void }) {
  const createDomain = useCreateDomain(tenantId);
...
function TlsBadge({ domain }: { readonly domain: { id: string; sslAutoRenew: number; tlsCertStatus?: string; tlsCertIssuer?: string | null; tlsCertExpiresAt?: string | null; tlsCertWildcard?: boolean } }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium cursor-default ${styles[status] ?? styles.none}`}
      title={tooltip}
