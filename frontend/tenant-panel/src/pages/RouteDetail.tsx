...
export default function RouteDetail() {
  ...
  const { data: routeData, isLoading, isError } = useRouteDetail(tenantId ?? undefined, routeId);
  ...
  return (
    <div className="space-y-6" data-testid="route-detail-page">
      ...
      {/* Tab content */}
      {activeTab === 'redirects' && <RedirectsTab tenantId={tenantId!} routeId={routeId!} route={route} dnsMode={dnsMode} />}
      {activeTab === 'security' && <SecurityTab tenantId={tenantId!} routeId={routeId!} route={route} />}
      {activeTab === 'access-control' && (
        <AccessControlTab tenantId={tenantId!} routeId={routeId!} hostname={route.hostname} />
      )}
      {activeTab === 'advanced' && <AdvancedTab tenantId={tenantId!} routeId={routeId!} route={route} />}
    </div>
  );
}
...
function RedirectsTab({ tenantId, routeId, route, dnsMode }: {
  readonly tenantId: string;
  readonly routeId: string;
  readonly route: RouteDetailResponse;
  readonly dnsMode: string | undefined;
}) {
  const updateRedirects = useUpdateRouteRedirects(tenantId, routeId);
...
function SecurityTab({ tenantId, routeId, route }: {
  readonly tenantId: string;
  readonly routeId: string;
  readonly route: RouteDetailResponse;
}) {
  const updateSecurity = useUpdateRouteSecurity(tenantId, routeId);
...
function AccessControlTab({
  ...
  return (
    <div className="space-y-8" data-testid="access-control-tab">
      <OidcSection tenantId={tenantId} routeId={routeId} hostname={hostname} />
      <MtlsSection tenantId={tenantId} routeId={routeId} />
      <ProtectedDirsSection tenantId={tenantId} routeId={routeId} />
    </div>
  );
}
...
function ProtectedDirsSection({ tenantId, routeId }: {
  readonly tenantId: string;
  readonly routeId: string;
}) {
  const { data: dirsData, isLoading } = useProtectedDirs(tenantId, routeId);
  const createDir = useCreateProtectedDir(tenantId, routeId);
  const deleteDir = useDeleteProtectedDir(tenantId, routeId);
          ...
          {dirs.map((dir) => (
            <div key={dir.id} data-testid={`protected-dir-row-${dir.id}`}>
              ...
              {expandedDirId === dir.id && (
                <ProtectedDirDetail tenantId={tenantId} routeId={routeId} dir={dir} />
              )}
            </div>
          ))}
        </div>
...
function ProtectedDirDetail({ tenantId, routeId, dir }: {
  readonly tenantId: string;
  readonly routeId: string;
  readonly dir: ProtectedDir;
}) {
  const updateDir = useUpdateProtectedDir(tenantId, routeId, dir.id);
  const { data: usersData, isLoading: usersLoading } = useDirUsers(tenantId, routeId, dir.id);
  const createUser = useCreateDirUser(tenantId, routeId, dir.id);
  const deleteUser = useDeleteDirUser(tenantId, routeId, dir.id);
  const toggleUser = useToggleDirUser(tenantId, routeId, dir.id);
...
function AdvancedTab({ tenantId, routeId, route }: {
  readonly tenantId: string;
  readonly routeId: string;
  readonly route: RouteDetailResponse;
}) {
  const updateAdvanced = useUpdateRouteAdvanced(tenantId, routeId);
