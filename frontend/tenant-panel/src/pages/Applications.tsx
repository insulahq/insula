      ...
      {!isLoading && !isError && (
        <>
          {filteredEntries.length === 0 ? (
            ...
            <TenantCatalogSections entries={filteredEntries} onCardClick={handleCardClick} />
          )}
...
function TenantCatalogSections({
  entries,
  onCardClick,
}: {
  ...
  return (
    <div className="space-y-6" data-testid="catalog-grid">
        ...
        <div data-testid="catalog-section-featured">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {featuredEntries.map((entry) => (
              <TenantCatalogCard key={`featured-${entry.id}`} entry={entry} onCardClick={onCardClick} />
            ))}
          </div>
        </div>
      )}
        ...
        <div data-testid="catalog-section-popular">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {popularEntries.map((entry) => (
              <TenantCatalogCard key={`popular-${entry.id}`} entry={entry} onCardClick={onCardClick} />
            ))}
          </div>
        </div>
      )}
      ...
      <div data-testid="catalog-section-all">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {entries.map((entry) => (
            <TenantCatalogCard key={`all-${entry.id}`} entry={entry} onCardClick={onCardClick} />
          ))}
        </div>
      </div>
    </div>
...
function InstalledTab({ onDeploy }: { readonly onDeploy: () => void }) {
  ...
  const updateDeployment = useUpdateDeployment(tenantId ?? undefined);
  const deleteDeployment = useDeleteDeployment(tenantId ?? undefined);
  const restoreDeployment = useRestoreDeployment(tenantId ?? undefined);
  const permanentDeleteDeployment = usePermanentDeleteDeployment(tenantId ?? undefined);
  const restartDeployment = useRestartDeployment(tenantId ?? undefined);
