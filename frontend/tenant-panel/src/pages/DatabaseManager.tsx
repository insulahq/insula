...
export default function DatabaseManager() {
  ...
  const { data: deploymentsData, isLoading: deploymentsLoading } = useDeployments(tenantId ?? undefined, { refetchInterval: 15_000 });
  ...
  const deployExportDb = useExportDatabase(isSqlite ? undefined : tenantId);
  const deployImportSql = useImportSql(isSqlite ? undefined : tenantId);
  const deployImportFromFile = useImportSqlFromFile(isSqlite ? undefined : tenantId);
  ...
  const sqliteExecuteQuery = useSqliteQuery(isSqlite ? tenantId : undefined);
  ...
  const sqliteExportDb = useSqliteExport(isSqlite ? tenantId : undefined);
  const sqliteImportSqlMutation = useSqliteImport(isSqlite ? tenantId : undefined);
  ...
  // Database management
  const createDbDatabase = useCreateDbDatabase(tenantId ?? undefined);
  const dropDbDatabase = useDropDbDatabase(tenantId ?? undefined);
  ...
  const createDbUser = useCreateDbUser(tenantId ?? undefined);
  const dropDbUser = useDropDbUser(tenantId ?? undefined);
  const setDbUserPassword = useSetDbUserPassword(tenantId ?? undefined);
...
function DeploymentStatsBar({ deployment }: { readonly deployment: { id: string; name: string; status: string; cpuRequest: string; memoryRequest: string } }) {
  const { tenantId } = useTenantContext();
  const { data } = useDeploymentLiveMetrics(tenantId ?? undefined, deployment.status === 'running' ? deployment.id : undefined);
