    ...
    <div className="space-y-6" data-testid="domain-detail-page">
      <div className="flex items-center gap-3">
        ...
        {domain.status === 'verified' && (
          <span className="inline-flex items-center gap-1 rounded-full bg-green-100 dark:bg-green-900/30 px-2.5 py-0.5 text-xs font-medium text-green-700 dark:text-green-300" data-testid="domain-verified-badge">
            <CheckCircle2 size={12} />
            Verified
          </span>
        )}
        {(domain.status === 'unverified' || domain.status === 'pending') && (
          <span className="inline-flex items-center gap-1 rounded-full bg-yellow-100 dark:bg-yellow-900/30 px-2.5 py-0.5 text-xs font-medium text-yellow-700 dark:text-yellow-300" data-testid="domain-unverified-badge">
            <AlertCircle size={12} />
            Unverified
          </span>
        )}
                ...
                <div className="mt-4 space-y-3" data-testid="verify-modal-failure">
                  ...
                  <div className="rounded-lg bg-gray-50 dark:bg-gray-700/50 p-3 text-xs text-gray-600 dark:text-gray-400">
                    {domain.dnsMode === 'cname' && (
                      <>
                        Update your A/AAAA or CNAME record at your DNS provider so{' '}
                        <span className="font-medium">{domain.domainName}</span> resolves to one of our cluster IPs.
                    ...
                    {domain.dnsMode === 'primary' && (
                      <>
                        Set your domain's nameservers to our platform nameservers at your registrar.
                        After updating, click <span className="font-medium">Verify Now</span> to recheck.
                    ...
                    {domain.dnsMode === 'secondary' && (
                      <>
                        Ensure your primary DNS server is configured to allow AXFR zone transfers to this platform.
                        After updating, click <span className="font-medium">Verify Now</span> to recheck.
                      </>
                    )}
                  </div>
                  {(() => {
                    ...
                    return cacheAt ? (
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        Last checked: {cacheAt.toLocaleString()}
                      </div>
                    ) : null;
                  })()}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" data-testid="delete-domain-modal">
          <div className="w-full max-w-xl rounded-xl bg-white dark:bg-gray-800 p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            {/*
              ...
              GET /api/v1/tenants/:cid/domains/:did/delete-preview so tenants
...
function RoutingTab({ tenantId, domainId, domainName, dnsMode }: {
  ...
  const { data: customDeploymentsData } = useCustomDeployments(tenantId);
...
function DnsTab({ tenantId, domainId }: { readonly tenantId: string; readonly domainId: string }) {
  ...
  const updateRecord = useUpdateDnsRecord(tenantId, domainId);
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm" data-testid="dns-records-section">
      ...
      {showSyncModal && (
        <SyncRecordsModal tenantId={tenantId} domainId={domainId} onClose={() => setShowSyncModal(false)} />
      )}
    </div>
  );
}
...
function SyncRecordsModal({ tenantId, domainId, onClose }: {
  readonly tenantId: string;
  readonly domainId: string;
  readonly onClose: () => void;
}) {
  const { data: diffData, isLoading, isFetching, isError, refetch } = useDnsRecordDiff(tenantId, domainId, true);
  const pullRecord = usePullDnsRecord(tenantId, domainId);
  const pushRecord = usePushDnsRecord(tenantId, domainId);
