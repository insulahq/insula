...
export default function ZrokProviders() {
  ...
  const { data: providers, isLoading } = useZrokProviders(tenantId ?? undefined);
  const deleteMut = useDeleteZrokProvider(tenantId ?? '');
...
function ProviderModal({ mode, provider, tenantId, onClose }: ModalProps) {
  const createMut = useCreateZrokProvider(tenantId);
  const updateMut = useUpdateZrokProvider(tenantId, provider?.id ?? '');
