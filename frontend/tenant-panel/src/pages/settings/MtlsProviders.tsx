...
export default function MtlsProviders() {
  ...
  const { data: providers, isLoading } = useMtlsProviders(tenantId ?? undefined);
  const deleteMut = useDeleteMtlsProvider(tenantId ?? '');
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 max-w-3xl">
        Reusable CA bundles for ingress-level tenant-cert authentication. Upload an existing CA
      ...
      {modal?.mode === 'upload' && tenantId && (
        <UploadOrEditModal mode="upload" provider={null} tenantId={tenantId} onClose={() => setModal(null)} />
      )}
      {modal?.mode === 'edit' && tenantId && (
        <UploadOrEditModal mode="edit" provider={modal.provider} tenantId={tenantId} onClose={() => setModal(null)} />
      )}
      {modal?.mode === 'generate' && tenantId && (
        <GenerateModal tenantId={tenantId} onClose={() => setModal(null)} />
      )}
      {modal?.mode === 'issue' && tenantId && (
        <IssueCertModal provider={modal.provider} tenantId={tenantId} onClose={() => setModal(null)} />
      )}
      ...
      {(providers ?? []).some((p) => p.canIssue) && (
        <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 shadow-sm">
          ...
          <p className="mt-1 mb-3 text-sm text-gray-600 dark:text-gray-400">
            Mint a fresh tenant cert from a provider that has a CA key on file. The private key
      ...
      {tenantId && (providers ?? [])
        .filter((p) => p.canIssue)
        .map((p) => (
          <MtlsCertificatesPanel key={p.id} tenantId={tenantId} provider={p} />
        ))}
...
function UploadOrEditModal({ mode, provider, tenantId, onClose }: UploadModalProps) {
  const createMut = useCreateMtlsProvider(tenantId);
  const updateMut = useUpdateMtlsProvider(tenantId, provider?.id ?? '');
...
function GenerateModal({ tenantId, onClose }: { readonly tenantId: string; readonly onClose: () => void }) {
  const createMut = useCreateMtlsProvider(tenantId);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" data-testid="mtls-generate-modal">
      <div className="w-full max-w-lg rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-xl">
        ...
        <p className="mt-1 mb-4 text-sm text-gray-600 dark:text-gray-400">
          ...
          private key never leaves. Use the &quot;Issue User Cert&quot; action to mint tenant
...
function IssueCertModal({
  ...
  const issueMut = useIssueMtlsCert(tenantId, provider.id);
  const [commonName, setCommonName] = useState('');
  const [organization, setOrganization] = useState('');
        ...
        {!issued ? (
          <form onSubmit={onSubmit} className="mt-4 space-y-3">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              The server signs a fresh tenant cert against this CA. The <strong>private key</strong> is
              ...
              {wantPkcs12 && (
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    accept passwordless .p12 imports; older tenants may prompt and accept an
