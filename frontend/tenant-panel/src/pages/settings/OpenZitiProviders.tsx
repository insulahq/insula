...
export default function OpenZitiProviders() {
  ...
  const { data: providers, isLoading } = useZitiProviders(tenantId ?? undefined);
  const deleteMut = useDeleteZitiProvider(tenantId ?? '');
...
function ProviderModal({ mode, provider, tenantId, onClose }: ModalProps) {
  const createMut = useCreateZitiProvider(tenantId);
  const updateMut = useUpdateZitiProvider(tenantId, provider?.id ?? '');
        ...
        <form onSubmit={onSubmit} className="mt-4 grid grid-cols-1 gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              ...
              for a long-lived tenant cert on first use.
            </p>
          </div>
