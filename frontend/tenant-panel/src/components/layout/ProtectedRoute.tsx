...
export default function ProtectedRoute({ children }: { readonly children: React.ReactNode }) {
  ...
  // Reject admin panel users from tenant panel (unless impersonating)
  if (user?.panel === 'admin') {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4 p-8">
        <p className="text-lg font-medium text-gray-900">Access Denied</p>
        <p className="text-sm text-gray-500">This portal is for tenants only. Admins can use the impersonation feature from the admin panel.</p>
      </div>
    );
  }
