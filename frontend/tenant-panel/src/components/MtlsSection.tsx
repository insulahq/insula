...
import type { IngressMtlsConfigInput } from '@insula/api-contracts';
...
export default function MtlsSection({ tenantId, routeId }: Props) {
  const { data: existing, isLoading } = useIngressMtls(tenantId, routeId);
  const { data: providers } = useMtlsProviders(tenantId);
  const upsert = useUpsertIngressMtls(tenantId, routeId);
  const remove = useDeleteIngressMtls(tenantId, routeId);
      ...
      {isOpen && (
        <form onSubmit={onSubmit} className="border-t border-gray-200 dark:border-gray-700 p-5 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Require incoming HTTPS tenants to present a valid certificate signed by your trusted CA.
          ...
          <fieldset className="rounded-lg border border-gray-200 dark:border-gray-700 p-3 space-y-3">
            ...
            <label className="flex items-center justify-between">
              <span className="text-sm text-gray-900 dark:text-gray-100">
                Full cert PEM as <code>ssl-tenant-cert</code> (URL-encoded)
              </span>
