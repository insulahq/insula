...
export function PrivateRegistryPanel({ tenantId, deploymentId, deploymentName, onClose }: Props) {
  const { data, isLoading, refetch } = usePullCredential(tenantId, deploymentId);
  const attach = useAttachPullCredential(tenantId);
  const revoke = useRevokePullCredential(tenantId);
