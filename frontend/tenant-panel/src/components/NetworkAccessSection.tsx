...
export default function NetworkAccessSection({ tenantId, deploymentId, deploymentName }: Props) {
  const { data: existing, isLoading } = useDeploymentNetworkAccess(tenantId, deploymentId);
  const { data: zitiProviders } = useZitiProviders(tenantId);
  const { data: zrokProviders } = useZrokProviders(tenantId);
  const upsert = useUpsertDeploymentNetworkAccess(tenantId, deploymentId);
