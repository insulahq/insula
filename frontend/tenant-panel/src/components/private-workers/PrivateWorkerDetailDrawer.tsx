...
function AuditTable({ tenantId, workerId }: AuditTableProps) {
  const { data, isLoading } = usePrivateWorkerAudit(tenantId, workerId, 50);
...
export default function PrivateWorkerDetailDrawer({
  ...
  const { data, isLoading, error } = usePrivateWorker(tenantId, workerId);
  const rotate = useRotatePrivateWorker(tenantId);
  const revoke = useRevokePrivateWorker(tenantId);
  const remove = useDeletePrivateWorker(tenantId);
          ...
          {worker && (
            <>
              ...
              <section className="space-y-2">
                ...
                <AuditTable tenantId={tenantId} workerId={workerId} />
              </section>
            </>
          )}
        </div>
