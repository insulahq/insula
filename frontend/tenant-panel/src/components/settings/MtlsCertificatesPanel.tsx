...
export default function MtlsCertificatesPanel({ tenantId, provider }: MtlsCertificatesPanelProps) {
  ...
  const certsQuery = useMtlsCertificates(tenantId, provider.id, statusFilter);
  const crlQuery = useMtlsCrlMetadata(tenantId, provider.id);
  ...
  async function downloadCertPem(cert: CertificateResponse) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/tenants/${tenantId}/mtls-providers/${provider.id}/certificates/${cert.id}/pem`,
        {
          headers: {
      ...
      a.href = url;
      const safeCn = cert.subjectCn.replace(/[^A-Za-z0-9._-]+/g, '_').slice(0, 64) || 'tenant';
  ...
  async function downloadCrl() {
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/tenants/${tenantId}/mtls-providers/${provider.id}/crl.pem`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('auth_token') ?? ''}`,
          },
...
function RevokeModal({
  ...
  const revokeMut = useRevokeMtlsCertificate(tenantId, providerId);
...
function UnrevokeModal({
  ...
  const unrevokeMut = useUnrevokeMtlsCertificate(tenantId, providerId);
...
function DeleteCertModal({
  ...
  const deleteMut = useDeleteMtlsCertificate(tenantId, providerId);
