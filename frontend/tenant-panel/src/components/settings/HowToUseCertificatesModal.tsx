      ...
      <div className="w-full max-w-3xl rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-xl max-h-[90vh] flex flex-col">
        <header className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 p-5">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
            <HelpCircle size={18} /> How to use your tenant certificate
          </h2>
          <button
            type="button"
            onClick={onClose}
...
function CLI() {
  return (
    <>
      ...
      <pre className="font-mono text-[11px] bg-gray-100 dark:bg-gray-900 rounded p-2 overflow-x-auto">{`curl --cert tenant.pem --key tenant.key https://protected.example.com/api/data`}</pre>
      <p className="text-xs font-semibold mt-2">wget:</p>
      <pre className="font-mono text-[11px] bg-gray-100 dark:bg-gray-900 rounded p-2 overflow-x-auto">{`wget --certificate=tenant.pem --private-key=tenant.key https://protected.example.com/`}</pre>
      <p className="text-xs font-semibold mt-2">openssl s_tenant (handshake debug):</p>
      <pre className="font-mono text-[11px] bg-gray-100 dark:bg-gray-900 rounded p-2 overflow-x-auto">{`openssl s_tenant -connect protected.example.com:443 \\
  -cert tenant.pem -key tenant.key -servername protected.example.com`}</pre>
...
const opts = {
  cert: readFileSync('tenant.pem'),
  key:  readFileSync('tenant.key'),
};
...
r = requests.get('https://protected.example.com/',
                 cert=('tenant.pem', 'tenant.key'))`}</pre>
