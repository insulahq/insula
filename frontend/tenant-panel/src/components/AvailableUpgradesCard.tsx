...
// ─── Hooks ──────────────────────────────────────────────────────────────────
// Local copies — the tenant-panel has its own apiFetch and hook conventions
// (separate package, no shared admin hook dir). These mirror the admin hooks
// in functionality but are scoped to tenant-panel.

function useAvailableUpgrades(tenantId: string, deploymentId: string) {
  return useQuery({
    queryKey: ['available-upgrades', tenantId, deploymentId],
    queryFn: () =>
...
function useUpgradeVersion(tenantId: string, deploymentId: string) {
