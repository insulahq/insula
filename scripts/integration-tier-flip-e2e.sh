...
if [[ "${TENANT_NODE_COUNT:-0}" -lt 3 ]]; then
  ...
  # + wait for status=active before any tenant-scoped op (the HA-flip PATCH).
  provision_tenant "$CID" || { fail "tier-flip/single-node guard: client provisioning failed"; exit 1; }
...
RESP=$(api POST "/tenants" "{\"name\":\"$COMPANY\",\"primary_email\":\"tier-e2e-$STAMP@example.test\",\"plan_id\":\"$PLAN_ID\",\"region_id\":\"$REGION_ID\",\"storage_tier\":\"local\"}")
CID=$(echo "$RESP" | python3 -c "import json,sys;print(json.load(sys.stdin)['data']['id'])" 2>/dev/null)
...
provision_tenant "$CID" || { fail "tier-flip: client provisioning failed"; exit 1; }
ok "client active"
