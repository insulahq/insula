...
STANDBY_CANDIDATE=$(run_kubectl "kubectl get node -l 'platform.phoenix-host.net/mail-standby=true,platform.phoenix-host.net/node-role=server' -o jsonpath='{.items[*].metadata.name}' 2>/dev/null" | tr ' ' '\n' | grep -v -F "$ACTIVE_NODE" | head -1)
