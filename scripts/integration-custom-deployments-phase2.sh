...
cleanup() {
  ...
  api_status POST "/admin/tenants/bulk" \
    "{\"client_ids\":[\"$TENANT_ID\"],\"action\":\"reactivate\"}" >/dev/null 2>&1 || true
  ...
  cnt=$(remote_kubectl get all,configmap,secret -n "$TENANT_NS" \
    -l "platform.example.test/e2e-phase2=$STAMP" \
  ...
  if [[ "$cnt" -gt "0" ]]; then
    ...
    remote_kubectl delete all,configmap,secret -n "$TENANT_NS" \
      -l "platform.example.test/e2e-phase2=$STAMP" 2>/dev/null || true
  fi
...
")
  ...
  if wait_pod_running "$TENANT_NS" "app=$name-writer" 120; then
    ...
    remote_kubectl get pods -n "$TENANT_NS" -l "platform.example.test/deployment-id=$id" -o wide 2>/dev/null || true
    return
  fi
...
")
  ...
  # Restore the client.
  status=$(api_status POST "/admin/tenants/bulk" \
    "{\"client_ids\":[\"$TENANT_ID\"],\"action\":\"reactivate\"}")
  if [[ "$status" == "200" ]]; then
    pass "T7: POST /admin/tenants/bulk {action:reactivate} → 200"
  else
...
")
  ...
  bundle_resp=$(api POST "/admin/tenant-bundles" \
    "{\"tenantId\":\"$TENANT_ID\",\"targetConfigId\":\"$backup_cfg_id\",\"components\":{\"files\":false,\"mailboxes\":false,\"config\":true,\"secrets\":false}}")
...
" 2>/dev/null || echo "?")
  ...
  if [[ "$http" == "422" || "$http" == "400" ]]; then
    ...
    replica_fail_reason=$(remote_kubectl get deploy -n "$TENANT_NS" \
      -l "platform.example.test/deployment-id=$dep_id" \
...
scenario_allow_root_endpoint() {
  ...
  local status
  status=$(api_status PATCH "/admin/tenants/$TENANT_ID/custom-deployments/$dep_id/allow-root" '{"allowRoot":true}')
  if [[ "$status" == "200" ]]; then
...
" 2>/dev/null || echo "")
  ...
  # 3) Revoke
  status=$(api_status PATCH "/admin/tenants/$TENANT_ID/custom-deployments/$dep_id/allow-root" '{"allowRoot":false}')
  if [[ "$status" == "200" ]]; then
  ...
  anon_status=$(curl -sk -o /dev/null -w "%{http_code}" \
    -X PATCH "$ADMIN_HOST/api/v1/admin/tenants/$TENANT_ID/custom-deployments/$dep_id/allow-root" \
