...
# when the snapshot carries the `platform.phoenix-host.net/barman-promote`
...
if [[ -f "$PG_RESTORE_SVC" ]]; then
  ...
  if ! grep -q "platform.phoenix-host.net/barman-promote" "$PG_RESTORE_SVC"; then
    echo "FAIL: postgres-restore/service.ts must reference the 'platform.phoenix-host.net/barman-promote' label (must match barman-restore/service.ts:takeLonghornSnapshotOfRestoredCluster spelling)"
    FAILED=1
  fi
fi
...
if ! grep -q "'platform.phoenix-host.net/barman-promote': 'true'" "$SERVICE"; then
  echo "FAIL: barman-restore/service.ts must label the Longhorn snapshot with platform.phoenix-host.net/barman-promote=true (sets the bypass flag postgres-restore/service.ts checks)"
  FAILED=1
fi
