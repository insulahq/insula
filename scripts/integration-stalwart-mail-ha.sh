...
SERVER_ROLE_LABEL='platform.example.test/node-role=server'
...
if [[ -z "$STALWART_DOMAIN" ]]; then
  echo "ERROR: STALWART_DOMAIN env var is required (e.g. mail.staging.example.test)" >&2
  exit 2
fi
