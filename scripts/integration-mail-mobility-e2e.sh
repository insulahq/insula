...
#
# Usage:
#   SSH_HOST=root@staging1.phoenix-host.net \
...
SSH_HOST=${SSH_HOST:-root@staging1.phoenix-host.net}
...
NODES_SERVER=$(kubectl get node -l platform.phoenix-host.net/node-role=server -o jsonpath='{.items[*].metadata.name}')
NODES_WORKER=$(kubectl get node -l platform.phoenix-host.net/node-role!=server -o jsonpath='{.items[*].metadata.name}' 2>/dev/null || echo "")
...
phase_H() {
  ...
  standby_candidate=$(kubectl get node -l 'platform.phoenix-host.net/mail-standby=true,platform.phoenix-host.net/node-role=server' -o jsonpath='{.items[*].metadata.name}' 2>/dev/null | tr ' ' '\n' | grep -v -F "$ACTIVE_NODE" | head -1)
  ...
  ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "${ACTIVE_NODE}.phoenix-host.net" 'systemctl stop k3s' 2>&1 | head -3
  ...
  ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "${ACTIVE_NODE}.phoenix-host.net" 'systemctl start k3s' 2>&1 | head -3
