...
#
# Usage:
#   SOURCE_ADMIN_HOST=https://admin.staging.example.test \
#   SOURCE_ADMIN_PASSWORD=<...> \
#   TARGET_VM_HOST=testing.example.test \
#   TARGET_VM_DOMAIN=testing.example.test \
...
# run yet, so the K8s node label `insula.host/node-role`
...
ssh "${SSH_OPTS[@]}" "${TARGET_SSH[@]}" \
  "KUBECONFIG=/etc/rancher/k3s/k3s.yaml; \
   kubectl label nodes --all insula.host/node-role=server --overwrite >/dev/null; \
