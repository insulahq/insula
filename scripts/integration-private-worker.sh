...
phase1_provision() {
  ...
  resp=$(api POST "/tenants" "$(jq -nc \
    --arg name "$company" \
    --arg email "pw-e2e-$stamp@example.test" \
    --arg plan "$plan_id" \
...
phase3_traffic() {
  ...
  local dom_resp did
  dom_resp=$(api POST "/tenants/$cid/domains" "$(jq -nc --arg h "$host" '{domain_name:$h, dns_mode:"cname"}')")
  ...
  if [[ -z "$did" ]]; then
    ...
    dom_resp=$(api POST "/tenants/$cid/domains" "$(jq -nc --arg h "$apex" '{domain_name:$h, dns_mode:"cname"}')")
    did=$(echo "$dom_resp" | jq -r '.data.id // empty')
  fi
  if [[ -z "$did" ]]; then
  ...
  route_resp=$(api POST "/tenants/$cid/domains/$did/routes" "$(jq -nc \
    --arg h "$host" \
    --arg pwid "$wid" \
...
phase4_revoke() {
  ...
  local resp
  resp=$(api POST "/tenants/$cid/private-workers/$wid/revoke" "{}")
  local status
