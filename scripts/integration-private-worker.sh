...
#   AGENT_IMAGE             ghcr.io/insulahq/private-worker-agent:latest
...
AGENT_IMAGE="${AGENT_IMAGE:-ghcr.io/insulahq/private-worker-agent:latest}"
...
phase1_provision() {
  ...
  local wid pw_token docker_run docker_compose
  ...
  ok "private worker created workerId=$wid (token captured)"
  echo "$wid" > "$STATE_WID"
