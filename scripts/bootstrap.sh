...
FIREWALL TRUST (always-on set mode):
  ...
  --pre-enroll-peer <ip>
                         ...
                         only (no CIDR; sets are type ipv*_addr):
                           --pre-enroll-peer 10.1.0.4
                           --pre-enroll-peer 192.0.2.116,192.0.2.56
...
spec:
  ...
  patches:
    ...
    # forever (caught live on the DEV cluster: reconciler logged
    # "unsuspended" while the CronJobs stayed suspended). suspend is
    # optional (defaults false); a fresh cluster creates them
    # unsuspended and the bridge's boot tick suspends them within
...
create_platform_configmap() {
  ...
  # Stalwart's ACME directory, read by stalwart-domain-reconciler.
  #
  # A DEDICATED ConfigMap, deliberately not a platform-config key: platform-config
  # ships in k8s/base and is owned by Flux, which overwrites bootstrap's value on
  # the next reconcile. That cost VM run 1f0c03dd its mail cert — bootstrap wrote
  # the key, Flux reconciled it away, and the reconciler then created a permanent
  # LET'S ENCRYPT provider because AcmeProvider.directory is read-only once set.
  # Nothing in k8s/ ships platform-mail-acme, so what bootstrap writes here stays.
  #
  # Only written when non-empty: an absent ConfigMap leaves the optional env unset
  # and the reconciler uses its own public-LE default, exactly as before.
  if [[ -n "${STALWART_ACME_DIRECTORY:-}" ]]; then
    log "Seeding platform-mail-acme ConfigMap (directory=${STALWART_ACME_DIRECTORY})..."
    kctl create configmap platform-mail-acme \
      --namespace=platform \
      --from-literal=directory="${STALWART_ACME_DIRECTORY}" \
      --dry-run=client -o yaml | kctl apply -f -
  fi
    ...
    - name: configure
      ...
      args:
        - |
          ...
          # Stalwart returns a HASH ID (e.g. 'irb3soetkwaa') for the
          # created entry, NOT the creation key. The creation key
          # ('letsencrypt' below) is only a per-batch reference; the
          # canonical ID is generated by Stalwart and must be read back
          # via x:AcmeProvider/get to use in subsequent Domain.cert
          # management references (see step 5b).
          #
          # Contact email — uses STALWART_CONTACT_EMAIL when set,
          # otherwise derives from the PLATFORM_APEX. LE validates the
          # contact email's domain at account-create time and rejects
          # reserved/non-deliverable TLDs (.test, .invalid, .local).
          # On dev DinD (apex k8s-platform.test) this WILL fail unless
          ...
          ACME_PROVIDER_ID=\$(echo "\$ACME_GET_RESP" | \
            jq -r '.methodResponses[0][1].list[0].id // empty' 2>/dev/null)
            ...
            ACME_SET_RESP=\$(jmap_call "\$(jq -n --arg a "\$ACCT" --arg c "\${ACME_CONTACT_EMAIL}" --arg d "\${ACME_DIRECTORY}" \
              '{using:["urn:ietf:params:jmap:core","urn:stalwart:jmap"],
                methodCalls:[["x:AcmeProvider/set",
                  ...
                  "c0"]]}')")
            ACME_FAIL=\$(echo "\$ACME_SET_RESP" | jq -r '.methodResponses[0][1].notCreated.letsencrypt // empty' 2>/dev/null)
            if [ -n "\$ACME_FAIL" ]; then
              # FAIL LOUDLY — do NOT silently skip. The most common cause
              ...
              echo "  REMEDIATION: set STALWART_CONTACT_EMAIL=<real@email.com> and re-run bootstrap." >&2
              exit 1
            fi
            echo "\$ACME_SET_RESP" | jq -r '.methodResponses[0] | "\(.[0]): \(.[1] | keys[0])"'
            # Read back the assigned hash ID — REQUIRED for step 5b.
            ACME_GET_RESP=\$(jmap_call "\$(jq -n --arg a "\$ACCT" \
              '{using:["urn:ietf:params:jmap:core","urn:stalwart:jmap"],
                methodCalls:[["x:AcmeProvider/get",{accountId:\$a,ids:null,properties:["id"]},"c0"]]}')")
            ACME_PROVIDER_ID=\$(echo "\$ACME_GET_RESP" | \
              jq -r '.methodResponses[0][1].list[0].id // empty' 2>/dev/null)
            if [ -z "\$ACME_PROVIDER_ID" ]; then
              echo "ERROR: AcmeProvider/set succeeded but /get returned no id" >&2
              exit 1
            fi
            echo "AcmeProvider created (id=\${ACME_PROVIDER_ID})"
          fi
          ...
          jmap_call "\$(jq -n --arg a "\$ACCT" --arg did "\$DOMAIN_ID" --arg pid "\${ACME_PROVIDER_ID}" --arg san "\${STALWART_HOSTNAME}" \
            '{using:["urn:ietf:params:jmap:core","urn:stalwart:jmap"],
              methodCalls:[["x:Domain/set",
                ...
                "c0"]]}')" | jq -r '.methodResponses[0] | "\(.[0]): \(.[1] | keys[0])"'
          echo "Domain.certificateManagement = Automatic (acme=\${ACME_PROVIDER_ID}, cert=\${STALWART_DOMAIN})"
          ...
          # error, never acquiring a cert.
          #
          # NOT stalwart-cli (deprecated in 0.16). NOT x:Bootstrap/set
          # requestTlsCertificate=true (forbidden post-bootstrap). NOT
          # x:Action/set ReloadTlsCertificates (no-op).
          #
          # Idempotent: AcmeRenewal checks whether the cert exists and is
          # near expiry before contacting Let's Encrypt.
          jmap_call "\$(jq -n --arg a "\$ACCT" --arg did "\$DOMAIN_ID" \
            '{using:["urn:ietf:params:jmap:core","urn:stalwart:jmap"],
              methodCalls:[["x:Task/set",
                {accountId:\$a,create:{r:{"@type":"AcmeRenewal",domainId:\$did}}},
                "c0"]]}')" | jq -r '.methodResponses[0] | "\(.[0]): \(.[1] | keys[0])"'
...
bootstrap_stalwart_v016() {
  ...
  # mail, except optionally _system@<apex> for SYSTEM tenant). With the
  # new pattern, the Stalwart Domain that anchors the platform mail
  # cert IS the mail hostname itself; the cert covers Domain.name
  # directly with no SAN-prefix tricks. Operator can still add the
  # platform apex as a separate Stalwart Domain via the Email Domains
  # UI if they want SYSTEM-tenant mailboxes there.
  local stalwart_domain="${stalwart_hostname}"
