...
REPO_URL="https://github.com/insulahq/k8s-hosting-platform.git"
...
OPTIONS:
  ...
  --host-client-workloads <true|false>
                         ...
                         the platform.example.test/host-client-
                         ...
                         client workloads by default" (defaults ON).
...
parse_args() {
  ...
  # --host-client-workloads. apply_node_labels_and_taints will then
  # skip stamping the platform.example.test/host-client-workloads
  ...
  # explicitly with `--host-client-workloads=false`.
...
apply_node_labels_and_taints() {
  ...
  # host-client-workloads label + server-only taint: stamp ONLY when
  # the operator passed an explicit --host-client-workloads value.
  ...
  # reads the admin Setting `newServerHostsClientWorkloads` (defaults
  ...
  if [[ -z "$HOST_CLIENT_WORKLOADS" ]]; then
    log "Labelling ${node_name}: role=${NODE_ROLE} (host-client-workloads deferred to admin Setting via k8s-sync reconciler)"
  else
    log "Labelling ${node_name}: role=${NODE_ROLE}, host-client-workloads=${HOST_CLIENT_WORKLOADS} (explicit --host-client-workloads override)"
    kubectl label node "${node_name}" \
...
install_traefik() {
  ...
  # the pod is Running. Detected on staging.example.test 2026-05-15.
  #
    ...
    - name: configure
      ...
      args:
        - |
          ...
          # CRITICAL: Stalwart returns a HASH ID (e.g. 'irb3soetkwaa') for
          # the created entry, NOT the creation key. The creation key
          ...
          # Phase K live-test on staging surfaced this: passing the
          # creation-key as acmeProviderId silently leaves Domain.cert
          # = Manual because Stalwart can't resolve the reference.
          ...
          # AcmeProvider HASH ID + the mail-hostname SAN. The SAN key is
          # the leading subdomain ('mail' for 'mail.example.com').
          #
          # WHY: with certificateManagement set to Manual (the default
          # after x:Domain/set create), Stalwart never tries to acquire
          # certificates. Setting it here at bootstrap means a fresh
          # install gets a real LE cert automatically when step 5c fires.
