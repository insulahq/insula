...
if [[ -n "$REMOTE_HOST" ]]; then
  ...
  # backups. Caught on testing.phoenix-host.net 2026-05-21.
...
apply_node_labels_and_taints() {
  ...
  if [[ "$NODE_ROLE" == "worker" ]]; then
    ...
    log "    kubectl label node ${node_name} platform.phoenix-host.net/host-tenant-workloads=${HOST_CLIENT_WORKLOADS} --overwrite"
...
generate_platform_secrets() {
  ...
  if kctl get secret -n platform backup-target-key >/dev/null 2>&1; then
    ...
    kctl annotate secret -n platform backup-target-key \
      'platform.phoenix-host.net/role=backup-target-key' \
      'platform.phoenix-host.net/restore-tier=tier-1-platform' \
