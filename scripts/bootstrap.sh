...
PY
    ...
    # Rewrite hardcoded `staging.phoenix-host.net` → operator's --domain
    # across every overlay file. The staging overlay was authored against
    # phoenix-host.net deployment; operators using a different domain
    # (testing.phoenix-host.net, customer-foo.com, …) need every host /
    ...
    # `staging.phoenix-host.net` token.
    log "Rewriting overlay hostnames: staging.phoenix-host.net → ${PLATFORM_DOMAIN}..."
    grep -rl "staging.phoenix-host.net" "$overlay_dir" 2>/dev/null \
      | xargs -r sed -i "s/staging\\.phoenix-host\\.net/${PLATFORM_DOMAIN}/g"
