...
if [[ $NEG_ONLY -eq 0 && -n "${SESSION_ID:-}" ]]; then
  ...
  if $KUBECTL_CMD --namespace="$NAMESPACE" exec system-db-1 -c postgres -- psql -U postgres -d platform -t -A -c "SELECT 1" >/dev/null 2>&1; then
    ...
    $KUBECTL_CMD --namespace="$NAMESPACE" exec system-db-1 -c postgres -- psql -U postgres -d platform -t -A -c "UPDATE users SET last_step_up_at=NULL WHERE email='$EMAIL'" >/dev/null 2>&1
    ...
    if [[ -n "$SAVED_AT" ]]; then
      $KUBECTL_CMD --namespace="$NAMESPACE" exec system-db-1 -c postgres -- psql -U postgres -d platform -t -A -c "UPDATE users SET last_step_up_at='$SAVED_AT' WHERE email='$EMAIL'" >/dev/null 2>&1
    fi
  else
...
if [[ $NEG_ONLY -eq 0 && -n "${SESSION_ID:-}" ]]; then
  ...
  if [[ -n "$F_SESSION_ID" ]] \
     && $KUBECTL_CMD_F --namespace="$NAMESPACE" exec system-db-1 -c postgres -- psql -U postgres -d platform -t -A -c "SELECT 1" >/dev/null 2>&1; then
    OWNER_BEFORE="$($KUBECTL_CMD_F --namespace="$NAMESPACE" exec system-db-1 -c postgres -- \
      psql -U postgres -d platform -t -A -c "SELECT owner_replica FROM node_terminal_sessions WHERE id='$F_SESSION_ID'" 2>/dev/null | head -1)"
...
if [[ $NEG_ONLY -eq 0 && -n "${NODE_NAME:-}" ]]; then
  ...
  H_HAS_DB=0
  if $KUBECTL_CMD_H --namespace="$NAMESPACE" exec system-db-1 -c postgres -- psql -U postgres -d platform -t -A -c "SELECT 1" >/dev/null 2>&1; then
    H_HAS_DB=1
  else
  ...
  if [[ "$H_HAS_DB" == "1" ]]; then
    H_TA="$($KUBECTL_CMD_H --namespace="$NAMESPACE" exec system-db-1 -c postgres -- \
      psql -U postgres -d platform -t -A -c "SELECT terminate_after FROM node_terminal_sessions WHERE id='$H_SESSION_ID'" 2>/dev/null | head -1)"
  ...
  if [[ "$H_HAS_DB" == "1" ]]; then
    H_EXISTS="$($KUBECTL_CMD_H --namespace="$NAMESPACE" exec system-db-1 -c postgres -- \
      psql -U postgres -d platform -t -A -c "SELECT id FROM node_terminal_sessions WHERE id='$H_SESSION_ID2'" 2>/dev/null | head -1)"
    if [[ -z "$H_EXISTS" ]]; then
