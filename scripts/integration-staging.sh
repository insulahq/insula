...
#
# DNS PREREQ
#   *.staging.example.test CNAMEs to staging.example.test (which
...
")
  ...
  if [[ "${RESTORE_INCLUDE_MAILBOXES:-}" == "1" ]]; then
    ...
    local probe_image="ghcr.io/insulahq/hosting-platform/mail-backup-tools:latest"
...
#
# Prerequisites:
#   - staging Stalwart running on 192.0.2.56 (ports 587, 993)
