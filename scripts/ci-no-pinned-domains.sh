...
# broke passkey login on the new-domain server (RP_ID staging.example.test
...
for pattern in "${PATTERNS[@]}"; do
  ...
  matches=$(
    grep -rn "$pattern" . $INCLUDE_GLOBS $EXCLUDE_DIRS 2>/dev/null \
      ...
      `# Dev overlay (k8s/overlays/dind) uses literal k8s-platform.test for the` \
      ...
      | grep -vE '^\./k8s/overlays/dind/.*k8s-platform\.test' \
      || true
  )
  if [ -n "$matches" ]; then
